...
> **목적**: `webtoon-index`, `content-portal`, `polling-app`, `rotation-app` 그리고 이후 모든 형제 레포(웹 + 토스인앱 동시 제공 서비스)의 폴더 구조·레이어·공유 패키지·툴링을 **단일 표준**으로 수렴시킨다.
...
| 항목                      | webtoon-index                      | content-portal              | polling-app               | rotation-app                     |
| ------------------------- | ---------------------------------- | --------------------------- | ------------------------- | -------------------------------- |
...
**결론**: 네 레포가 "비슷하지만 다 다르다". 표준 정렬 시 기준은 **rotation-app(데이터 레이어·레이어 경계가 가장 현대적) + polling-app(shared/client 2단 분리) + content-portal(`@heejun/*` preset + `tsconfig.base`)** 의 장점 합집합.
...
- 앱 내부 도메인 폴더(`domains/`, `features/`, `entities/`)를 앱마다 두지 않는다 — 그건 `packages/`로 올린다. (rotation-app 웹은 `domains/`가 두꺼운데, 로직은 `client`로, 뷰만 `pages/`로 분해하는 방향.)
...
- 의존성은 **항상 한 방향**(apps → client → shared). eslint boundaries로 강제(rotation-app/webtoon에 이미 존재 → 표준화).
...
현재 네 레포가 제각각인 부분. 표준은 **polling-app의 shared+client 2단 분리**를 채택한다.

- **`shared`** = "노드/브라우저 어디서나 도는 순수 로직". 백엔드도 쓴다. **표준 패키징 = raw-src 워크스페이스 레시피**(빌드 스텝 無): `package.json`의 main/module/types → `./src/index.ts`, `tsconfig moduleResolution: bundler`. 서브패스(`./server` 등)로 브라우저세이프/노드전용 분리. (레퍼런스: webtoon `@toonspectrum/core`. 현재 polling-app/rotation-app는 tsup 빌드 → raw-src 정렬은 선택사항, 강제 아님.)
...
- 토스 `.ait` 번들러가 `workspace:*`를 못 풀므로, 토스는 `shared`/`client`를 **소스 경로 alias**로 참조(`packages/*/src`). polling-app의 `sync-toss-shared.mjs` + verify 게이트 패턴을 표준 채택.
...
- **content-portal**: `content`(데이터+util) → `shared`(데이터+zero-dep util) 유지 + `client` 신설(뷰모델·스토어). 토스 generate-JSON은 §짝문서의 "repository 어댑터"로 흡수.
- **polling-app**: 이미 `shared`+`client` ✓ — 표준의 레퍼런스. 토스 `lib/`에 흩어진 로직(format/keywords/pollSignal 중복)을 shared/client로 회수.
- **rotation-app**: `shared`만 있음 → `client` 신설. 웹 `domains/*/queries.ts`(TanStack 훅)·스토어를 `client`로 승격. 토스 bespoke API 래퍼 제거하고 client의 데이터 훅 재사용.
...
| eslint            | `@heejun/eslint-config@^5` (flat, boundaries 포함)               | webtoon=@4(업), **polling-app=인라인(도입)**                       |
| prettier          | `@heejun/prettier-config@^3`                                     | webtoon·polling-app 인라인(도입)                                   |
| tsconfig          | 루트 `tsconfig.base.json` + 앱/패키지 extends                    | content-portal만 있음 → 나머지 도입                                |
| 스타일(웹)        | Tailwind v4(`@tailwindcss/vite`, no config 파일) + 토큰 CSS 변수 | **polling-app=72KB 수기 CSS**(Tailwind 이관)                       |
| React             | 19.x + React Compiler(babel-plugin)                              | 전부 OK                                                            |
| Vite              | 웹 8.x / 토스 6.x(Granite 제약)                                  | 전부 OK — 의도된 차이                                              |
| ORM               | **Drizzle + Neon Postgres**                                      | **rotation-app=Prisma**(장기 선택, §5.4)                           |
| 검증              | nestjs-zod                                                       | 전부 OK                                                            |
| 데이터 레이어(웹) | **ky + TanStack Query v5**                                       | webtoon·polling-app·content-portal 도입(§5.5)                      |
| 라우팅            | React Router 7 (웹·토스 **둘 다**)                               | content-portal(웹 수동)·polling-app/rotation-app(토스 커스텀) 도입 |
...
### 5.2 polling-app — 우선순위 상 (preset 미적용 + CSS)
...
### 5.4 rotation-app — 우선순위 중 (데이터 레퍼런스지만 client 없음)
...
- 표준 = **`ky`(HTTP) + TanStack Query v5(서버 상태 캐시/무효화) + Zustand(클라 UI 상태)**. rotation-app가 레퍼런스.
- 데이터 훅은 전부 `client`의 `use<Domain>Query` / `use<Domain>Mutation`으로. 웹/토스 공유.
- 정적 데이터 앱(content-portal, webtoon catalog)도 동일 훅 시그니처를 쓰되 내부 DataSource만 다름(§짝 문서 §6).
