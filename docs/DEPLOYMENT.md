...
이 저장소에는 **빌드 단계가 없습니다.** 번들러·엔트리·`vite.config`가 없으며,
배포는 저장소 루트의 원본 파일을 그대로 발행합니다.

| 대상         | 출력             | 용도                                                             |
| ------------ | ---------------- | ---------------------------------------------------------------- |
| 검증기       | (없음)           | **CI/Netlify 게이트** — 문서 검증기만 실행, 산출물 생성 안 함    |
| Netlify 발행 | 저장소 루트(`.`) | **실제 서빙 대상** — 원본 `index.html` + `public/`을 그대로 배포 |

> Netlify의 `publish = "."`이고 배포 워크플로의 `--dir "."`이므로, 사이트는
> **저장소 루트의 원본 `index.html`**을 그대로 서빙합니다. 빌드 산출물을 만들지
> 않으므로 CSP에 nonce를 주입할 수 없는 이유도 이것입니다(원본 HTML을 손대지 않고 서빙).
...
```bash
pnpm install
# index.html을 브라우저로 직접 열어 미리보기(빌드/서버 불필요)
