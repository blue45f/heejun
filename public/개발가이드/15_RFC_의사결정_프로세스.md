...
| 용어        | 쉬운 뜻                                                 |
| ----------- | ------------------------------------------------------- |
| `워킹그룹`  | 여러 이해관계자가 모여 결정안을 검토하는 임시 운영 단위 |
| `RFC`       | 되돌리기 어려운 변경을 논의하는 제안서 형식             |
...
> 이 문서의 표준은 특정 협업 도구나 RFC 형식 자체가 아니라 **중요한 결정을 책임 있는 사람들과 충분한 맥락으로 검토하고, 승인된 결정을 이후에도 추적 가능한 기록으로 남기는 방식**입니다.
...
- `증거` : 제안서 diff, ADR 번호, PoC 결과, 반대 의견 처리 중 실제로 남긴 항목만 링크한다.
...
| :----------------------------------------------------------------- | :--------------------------------------------------------------------------------------- |
| 워킹그룹 구성 기준, 의사결정 제안서 필요 조건, ADR 보존, 철회 조건 | [04. 아키텍처](./04_아키텍처_설계_패턴.md), [14. 배포](./14_배포_프로세스_체크리스트.md) |
...
비유: 자동차 정비와 같습니다. 와이퍼 교체는 그 자리에서 하지만(`PR`), 엔진 교체는 정비 책임자, 견적, 대안, 일정이 필요합니다(`워킹그룹/제안서`).
...
```mermaid
gantt
  title 의사결정 운영 일정 (2주 cadence 예시)
...
- `승인 역할` : 제안서/ADR 작성자, 검토자, 운영 확인자를 분리해 기록한다.
