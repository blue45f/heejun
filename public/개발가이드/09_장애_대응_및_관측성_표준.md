...
- **SLO 기반 장애 기준**: P0/P1 기준을 단일 임계값으로 고정하고, error budget 소진률을 릴리즈 품질 게이트와 연결합니다.
- **관측 데이터 정합성**: trace-id, 요청ID, 사용자 세그먼트, 릴리즈 버전 정보를 end-to-end로 추적하도록 스키마를 고정합니다.
- **사후재발 방지 루프**: postmortem action item의 자동화 여부를 체크하고, 미적용 항목이 30일 내 반영되지 않으면 다음 배포를 block합니다.
- **운영 대응 훈련**: 장애 runbook을 월 1회 테이블탑으로 점검하고 실습 로그를 문서 evidence로 남깁니다.
...
- SLO 우선 운영: 에러 예산 소진률 임계치 접근 시 배포 속도를 낮추고 관측성 강화 태스크를 선행 수행합니다.
- 트랜잭션 추적 정합: 요청 ID와 배포 버전, 사용자 세그먼트를 함께 묶어 장애 원인 분석 시간을 단축합니다.
- 배포 직후 안전 구간: 론칭 후 30분 내 경보 임계치를 강화해 조기 대응 체계를 가동합니다.
- 사후조치 자동화: 미해결 action item은 다음 배포를 blocking하는 자동 규칙으로 관리합니다.
- 재발 방지 테이블: 반복 장애는 playbook으로 정형화해 유사 사건의 대응 시간을 일정 수준 이하로 제한합니다.

- 장애 의심 신호는 임계치 기반 경보만이 아니라 장애 패턴 군집도(동일 stacktrace 재발 빈도)로 함께 판단해 오탐을 낮춥니다.
- postmortem 액션 아이템은 테스트/알림/운영 runbook로 닫히는 구조를 강제하고, 미완료 시 배포 자동 차단 조건에 반영합니다.
- SLO 위반이 발견되면 기능 축소·피처 플래그 오프로 이어지는 보완조치를 24시간 내 실행 스케줄에 편성합니다.
- 장애 알림은 단발성보다 재발 패턴을 기준으로 우선순위를 부여해 재작업 비용을 줄이고 장기 RCA를 구조화합니다.
- 운영 지표 정합성 드리프트는 postmortem 시작 조건으로 설정해 원인 추적 지연을 선제 차단합니다.
- 장애 대응은 알람 빈도와 회복 시간 둘 다 지표화해, 반복 알람 구간은 자동 차단·조치 플랜과 연결합니다.
- 관측성 이벤트를 인시던트 템플릿과 조합해 운영자 판단 시간을 단축하는 보조 지침을 유지합니다.
- 탐지 지연이 큰 항목은 경고 임계치를 낮추고 초기 대응 프로세스에 대한 자동 할당 규칙을 둡니다.
- postmortem는 증거 누락 없이 완료 상태를 트래킹해 미해결 항목이 다음 배포 전 완결되게 합니다.
...
- **알람 노이즈 제어**: 단발 경보가 아닌 패턴 기반 경보만 운영 의사결정에 반영합니다.
- **사건 단절 규칙**: postmortem 미해결 항목은 다음 릴리즈의 승인 차단 조건으로 연결합니다.
...
- [ ] **알람 정책 정합성**: 임계 임팩트 알람이 runbook 실행으로 이어지는지 점검한다.
- [ ] **복구 연습 주기**: 월간 드릴 수행 로그를 추적하고 미이행 항목을 차단한다.