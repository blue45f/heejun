...
- `첫 적용` — keyboard, screen reader, ARIA, reduced motion 증거 중 하나를 실제 PR이나 운영 이슈에 붙이고, 변경 전 기준을 먼저 적는다.
- `증거 정리` — axe report, keyboard recording, SR note, contrast check를 같은 작업 기록에 남긴다.
- `재점검` — critical a11y violation, keyboard trap, label 누락가 나아졌는지 30일 안에 확인하고 기준을 유지, 수정, 폐기 중 하나로 판정한다.

## 추천 항목 실행 기록 템플릿

- `작업` : keyboard, screen reader, ARIA, reduced motion 증거 적용 범위를 어느 화면, 패키지, 문서에 둘지 적는다.
- `증거` : axe report, keyboard recording, SR note, contrast check 중 실제로 남긴 항목만 링크한다.
- `판정` : 유지/수정/폐기 중 하나와 이유를 한 문장으로 남긴다.
- `다음 점검` : critical a11y violation, keyboard trap, label 누락를 다시 볼 날짜와 담당자를 지정한다.
...
const WCAG_TAG_MAP: Record<WcagLevel, string[]> = {
  ...
  AAA: ['wcag2a', 'wcag21a', 'wcag22a', 'wcag2aa', 'wcag21aa', 'wcag22aa', 'wcag2aaa', 'wcag21aaa'],
}
...
export function getMaxViolations(): number {
  ...
  switch (level) {
    case 'A':
      return 5 // preview: 경미한 위반 허용
    case 'AA':
      return 0 // staging: 위반 불허
    case 'AAA':
      return 0 // production: 위반 불허
  }
}
...
export function PageLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      ...
      <header role="banner">
        <nav aria-label="메인 네비게이션">{/* 네비게이션 링크들 */}</nav>
      </header>
      ...
      <footer role="contentinfo">{/* 푸터 정보 */}</footer>
    </>
  )
}
```
...
// 분기별 증적 보고서 생성
async function generateQuarterlyReport(evidences: A11yEvidence[]): Promise<string> {
...
- `P1(7일 내)` — keyboard, screen reader, ARIA, reduced motion 증거 중 하나를 작은 변경 1건에 적용하고 증거(axe report)를 남긴다.
- `P2(30일 내)` — 접근성 기준을 팀 템플릿, 체크리스트, CI 중 한 곳에 고정한다.
- `P3(90일 내)` — critical a11y violation, keyboard trap, label 누락 추이를 보고 기준을 유지할지 조정할지 결정한다.
- `완료 기준` — 접근성 오너가 증거와 철회 조건을 확인했다는 기록을 남긴다.

## 추천 항목 실행 체크리스트

- [ ] `1단계(7일)` : keyboard, screen reader, ARIA, reduced motion 증거 적용 대상을 1개로 좁힌다.
- [ ] `2단계(30일)` : 증거(axe report, keyboard recording, SR note, contrast check)를 PR, ADR, 회고 중 한 곳에 연결한다.
- [ ] `3단계(60일)` : critical a11y violation, keyboard trap, label 누락가 기준 안에 들어왔는지 확인한다.
- [ ] `문제 대응` : 미달성 사유와 다음 조치, 중단 여부를 같은 기록에 남긴다.

## 추천 항목 실행 운영 규칙

- `실행 게이트` : 마우스 없이 핵심 flow가 가능한지 먼저 확인한다.
- `승인 체계` : 접근성 오너가 영향 범위와 rollback 담당자를 적용 전에 확인한다.
- `재개 조건` : 자동 검사와 수동 키보드/SR 확인이 맞으면 release gate에 올린다.
- `정지 조건` : critical violation이나 focus trap이 남으면 release를 막는다.
- `리스크 점수` : 핵심 flow 수, 사용자 영향, 법적 노출로 산정한다.
- `리더 승인자` : 접근성 리드가 최종 승인 책임을 맡는다.
- `승인 역할` : 접근성 작성자, 검토자, 운영 확인자를 분리해 기록한다.
- `재평가 주기` : 릴리스 전과 디자인 시스템 변경 후에 다시 검토한다.
