...
| **핵심 테마** | WCAG 2.2 (ISO/IEC 40500:2025), EAA, ADA Title II, WAI-ARIA 1.2 stable + ARIA 1.3 draft watch, prefers-* 미디어 쿼리, 자동 게이트 | **Update** | 2026.05 |
...
> - **ADA Title II**: ADA.gov 기준 2026.04.20 Interim Final Rule로 compliance date가 연장되어, 인구 5만 이상 공공기관은 2027.04.26, 5만 미만 및 특수구는 2028.04.26을 기준으로 관리합니다.
...
1. [AI 보조 접근성 감사 검증 시나리오](#1-ai-보조-접근성-감사-검증-시나리오)
2. [WAI-ARIA 안정 패턴 + ARIA 1.3 Draft Watch](#2-wai-aria-안정-패턴--aria-13-draft-watch)
3. [CSS prefers-* 미디어 쿼리 통합](#3-css-prefers--미디어-쿼리-통합)
4. [환경별 접근성 자동 게이트](#4-환경별-접근성-자동-게이트)
...
production baseline은 WAI-ARIA 1.2와 WAI-ARIA Authoring Practices 안정 패턴입니다. WAI-ARIA 1.3은 2026년 5월 기준 Working Draft이므로, 신규 속성은 접근성 트리, 브라우저, 스크린 리더 조합에서 검증한 뒤 제한적으로 사용합니다. 이 섹션은 Disclosure, Feed, Meter 같은 실무 패턴을 TypeScript로 구현하고, 키보드 인터랙션, 스크린 리더 호환성, 고대비 모드 대응을 함께 다룹니다.
...
2026년 접근성 자동화는 CI 게이트에서 IDE 단계로 확장되고 있다. 정적 검사, 런타임 검사, AI 보조 수정 제안을 함께 사용하되, AI가 만든 수정은 키보드 조작, 스크린 리더 흐름, WCAG 기준으로 다시 확인한다.
...
- **사용성 전환 기준**: 키보드 경로, focus 관리, 스크린리더 문맥의 3축 테스트를 릴리즈 게이트로 의무화합니다.
- **디자인/컴포넌트 계약화**: 모든 컴포넌트는 자동 생성되는 접근성 메타(필수 레이블, roles, live region)를 표준으로 둡니다.
- **국제 사용성 동시 고려**: RTL/다국어 텍스트/동적 메시지에서 순서·길이 변경을 고려한 접근성 점검을 반영합니다.
- **감소 환경 지원**: prefers-reduced-motion, 고대비, 폰트 확대 등에 대한 회귀 테스트 템플릿을 두어 예외 없이 운영합니다.
...
- 접근성 gate 배포 연동: keyboard/screen reader/focus 실패율을 배포 게이트 조건으로 고정합니다.
- 컴포넌트 접근성 계약: 필수 역할·라벨·알림 정책을 컴포넌트 API 문서와 테스트에 통합합니다.
- 국제화 영향 반영: 텍스트 길이·문장 순서 변화를 키보드 탐색, 터치 타깃, 포커스 흐름에 반영합니다.
- 저시력·인지 부하 지표화: reduced-motion, contrast, font scaling 성능 지표를 정량 추적합니다.
- 운영 연동: 접근성 실패를 운영 알림으로 자동 이송하고 이슈 해결시점을 기록합니다.

- 접근성 점검은 기능 변경 전후로 키보드 경로, 화면낭독기 문맥, 고대비 모드 3축으로 비교 분석합니다.
- 애니메이션/동적 콘텐츠는 동작 억제 옵션과 대체 텍스트 경로를 기본 제공해 사용성 회귀를 방지합니다.
- WCAG 체크는 완성 항목이 아니라 회귀 체크로 유지해 출시마다 동일 기준으로 재측정합니다.
- 접근성 지표는 기능 전환/실패율과 결합해 사용자 피로도를 줄이는 쪽으로 우선순위를 재구성합니다.
- 동등성 보장 항목은 언어·디바이스별로 분리해 회귀를 조기 발견하도록 체크리스트를 강화합니다.
- 접근성 향상 조치는 실제 사용자 유형별 체감 개선 지표로 검증해, 과도한 규격 준수만 남는 상태를 방지합니다.
- 키보드 포커스 경로와 스크린리더 문구의 회귀를 실사용자 시나리오 테스트에 고정합니다.
- 접근성 점검 실패는 우선 기능 중요도와 사용자 영향도로 분류해 대응 우선순위를 동적으로 조정합니다.
- 장애인·고령 사용자군 기준 시나리오로 모듈별 회귀 여부를 분기 검증합니다.
...
- **접근성 위험 분리**: 핵심 경로 접근성 실패는 배포 차단 항목으로 강제 분류합니다.
- **사용자군 우선 검증**: 고위험 접근성 이슈는 실사용자군 기반 검증을 선결 과제로 둡니다.
...
- [ ] **사용자군 회귀 점검**: 접근성 실패율 높은 사용자군별 보정 항목을 우선 처리한다.
- [ ] **검증 자동화**: 접근성 체크 자동화 파이프라인에서 오류를 누락 없이 수집한다.