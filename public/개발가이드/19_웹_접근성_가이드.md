# 19. 웹 접근성 가이드 (2026 Edition)

| 분류 | 핵심 기술 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [20. 디자인 시스템](./20_디자인_시스템_가이드.md), [07. 테스팅](./07_테스팅_가이드.md) | **AI 도구** | axe-core 4.10, Playwright, Claude Code |
| **핵심 테마** | WCAG 2.2 (ISO/IEC 40500:2025), EAA, ADA Title II, ARIA 1.3, prefers-* 미디어 쿼리, 자동 게이트 | **Update** | 2026.05 |

---

> **"접근성은 선택이 아니라, 모든 사용자가 동등하게 서비스를 경험할 수 있도록 보장하는 기본 품질 기준이다."**
> 본 가이드는 WCAG 2.2 AA/AAA 준수, EAA(2025.06.28 시행) 및 ADA Title II(2026.04.26 시한) 컴플라이언스, ARIA 1.3 / APG 2026 패턴 구현, prefers-* 미디어 쿼리 통합, CI/CD 자동 게이트까지 실무에 필요한 접근성 전 영역을 다룹니다. 소송/감사 대비 증적 체계와 AI 기반 자동 수정 제안까지 포함합니다.
>
> **2026년 규제 환경 변화 요약 (2026.05 시점)**
> - **WCAG 2.2**: 2025년 10월 ISO/IEC 40500:2025로 표준화 완료 (October 2023 권고안과 동일)
> - **EAA (European Accessibility Act)**: 2025년 6월 28일부터 EU 27개국 전역에서 강제 시행. EN 301 549가 WCAG 2.2를 포함하도록 업데이트 진행 중. 2026년 5월 기준 프랑스(가처분), 독일(UWG 사설 경고장), 스웨덴(PTS 규제 검사), 네덜란드(ACM 우선 감사) 등에서 실질 enforcement 진행 중 (섹션 12 사례 참고)
> - **ADA Title II**: 2026.04.20 DOJ가 Interim Final Rule 발표로 시한 1년 연기. **인구 5만 이상 자치단체는 2027.04.26**, **5만 미만 및 특수구는 2028.04.26**로 변경. ADA Title III(민간) 소송은 2025년 한 해에만 연방 법원에 3,117건 제기되어 전년 대비 +27% 증가 (섹션 12.2 참고)
> - **axe MCP Server (2026)**: Deque가 axe DevTools for Web 번들에 무료 포함. Claude Code/Cursor/GitHub Copilot/Windsurf/VS Code 등 MCP 호환 AI 에이전트의 IDE 내 1-클릭 접근성 수정 표준화 (섹션 11 참고)
...
2. [ARIA 1.3 / APG 2026 패턴 구현](#2-aria-13--apg-2026-패턴-구현)
...
11. [axe DevTools Linter + MCP Server 통합 (2026)](#11-axe-devtools-linter--mcp-server-통합-2026)
12. [2026 규제 운영 사례 박스](#12-2026-규제-운영-사례-박스)
13. [참고 자료](#13-참고-자료)
...
## 2. ARIA 1.3 / APG 2026 패턴 구현

ARIA 1.3 명세(2026년 W3C Candidate Recommendation 단계)와 Authoring Practices Guide 2026의 핵심 패턴인 Disclosure, Feed, Meter를 TypeScript로 구현한다. 각 패턴은 키보드 인터랙션, 스크린 리더 호환성, 고대비 모드 대응을 포함한다.

> **ARIA 1.3 주요 변경점**
> - `aria-description` 정식 채택 (2026년 모든 주요 스크린 리더 지원 완료)
> - `aria-braillelabel`, `aria-brailleroledescription`: 점자 디스플레이 사용자 지원
> - `aria-actions`: 컨텍스트 액션 노출 (베타)
> - `role="suggestion"`, `role="mark"`, `role="comment"`: 편집 컨텍스트용 신규 롤
> - 기존 `aria-labelledby`/`aria-describedby` 우선순위 규칙 명확화
...
> **글로벌 서비스 컴플라이언스 가이드 (2026.05 시점)**
> - **EU 시장 서비스**: EAA(2025.06.28 발효) 의무 적용. e-commerce, 은행, 미디어, 운송, 통신, 전자책 등 거의 모든 디지털 서비스가 대상. WCAG 2.1 AA를 최소 baseline, WCAG 2.2 AA를 권장. 2026년 5월 기준 프랑스/독일/스웨덴/네덜란드 등에서 실질 enforcement 활동 진행 중 (섹션 12 참고).
> - **US 정부 협력 서비스**: ADA Title II Web Accessibility Rule이 **2026.04.20 DOJ Interim Final Rule로 1년 연장**. 인구 5만 이상은 **2027.04.26**, 5만 미만은 **2028.04.26**이 새 시한. 주/지방정부 협력사는 여전히 WCAG 2.1 AA 의무이며, 추가 1년은 점진적 도입 기회일 뿐 면제가 아니다.
...
## 11. axe DevTools Linter + MCP Server 통합 (2026)

2026년 들어 Deque의 axe Platform이 두 가지 큰 변화를 가져왔다. 첫째, axe DevTools Linter 4.x가 44개 접근성 룰로 React/Vue/Angular/HTML/Markdown을 정적으로 검사한다. 둘째, **axe MCP Server**가 axe DevTools for Web 번들에 추가 비용 없이 포함되어, GitHub Copilot, Cursor, Claude Code, Windsurf, VS Code 등 MCP 호환 AI 에이전트가 IDE 안에서 직접 접근성 수정을 적용할 수 있게 되었다. 본 가이드의 CI 게이트가 "위반 차단"에 초점을 둔다면, axe Linter/MCP는 "위반이 코드에 들어오기 전 IDE에서 차단"하는 한 단계 더 앞선 방어선이다.
...
# 적용 도구: VS Code 확장(deque-systems.vscode-axe-linter), axe-linter CLI, GitHub Action
global:
  ...
  # axe Linter에게 "이 컴포넌트는 사실상 button/img/a이다"라고 알린다.
...
### 11.2 PR 차단용 axe-linter GitHub Action
...
axe MCP Server는 MCP 호환 AI 에이전트(Claude Code, Cursor, GitHub Copilot, Windsurf, VS Code)가 IDE 내부에서 axe Platform의 자동 룰과 Intelligent Guided Tests에 직접 접근하도록 한다. 본 가이드의 AI 자동 수정 제안(섹션 5.2의 `a11y-auto-fix.ts`)과 결합하면, "감사 보고서 -> 사람이 PR 작성"이 아니라 "IDE에서 즉시 1-클릭 적용"이 가능해진다.

```jsonc
// .claude/mcp.json 또는 ~/.config/claude/mcp.json (Claude Code 기준)
// 용도: axe MCP Server를 Claude Code/Cursor/Windsurf에서 사용하도록 등록
{
  "mcpServers": {
    "axe": {
      ...
      "env": {
        // Deque 라이선스 키 (axe DevTools for Web 구독 필요)
...
> **권장 원칙:** AI 자동 수정은 항상 사람이 diff를 검토한 후 커밋한다. ARIA 1.3 신규 속성(`aria-description`, `aria-braillelabel`)이나 `forced-colors` 처리처럼 의도가 중요한 영역은 AI가 단순 패턴 매칭으로 잘못 수정할 수 있다.

---

## 12. 2026 규제 운영 사례 박스

본 가이드의 컴플라이언스 매핑(섹션 10의 환경별 게이트)이 실제 2026년 시장에서 어떻게 작동하는지, 공개된 사례를 모아둔다. 모든 사례는 2026년 5월 시점의 공개 보도와 규제기관 발표에 기반한다.

### 12.1 European Accessibility Act (EAA) 시행 후 경과

> **사례 1 -- 프랑스, 대형 유통사 응급가처분 (2025.11)**
> EAA가 2025.06.28 시행된 직후, EU 회원국 중 가장 빠르게 사법 절차가 진행된 곳은 프랑스다. 시민단체가 Carrefour, Auchan, Leclerc 등 대형 유통사의 e-commerce 사이트가 접근성 요건을 충족하지 못한 점을 들어 파리 지방법원(Tribunal judiciaire de Paris)에 응급가처분(injunction)을 신청했다. 2026년 4월 시점 판결이나 벌금 부과는 없었지만, "법적 통지(formal notice)" 자체가 회사들에 강한 압박이 되었다. 본 가이드 섹션 10의 "Staging 단계에서 WCAG 2.1 AA를 머지 차단 게이트로 강제" 원칙은 이 같은 응급가처분 리스크를 사전 차단하기 위한 설계이다.

> **사례 2 -- 독일, BFSG와 경쟁법 결합 경고장 (2025.08 이후)**
> 독일은 EAA를 BFSG(Barrierefreiheitsstärkungsgesetz)로 국내법화했다. 시행 직후 몇 주 만에 e-commerce 운영자들에게 사설 경고장(Abmahnung)이 쇄도하기 시작했다. 흥미로운 점은 이 경고장 대부분이 BFSG 자체가 아니라 독일 경쟁법(UWG)을 근거로 한다는 점이다. 즉, "경쟁사 대비 접근성이 떨어지는 것이 부정경쟁"이라는 논리다. 본 가이드의 "VPAT + ADR + 수정 일정(due diligence 증적)" 보관 원칙은 이런 사설 경고장에 대응할 때 "성실히 개선 중"을 증명하는 핵심 자료가 된다.

> **사례 3 -- 스웨덴 PTS, 첫 규제 검사 착수 (2025년 하반기)**
> 스웨덴 우편통신청(PTS)이 EAA 시행 이후 e-commerce 운영자를 대상으로 첫 규제 검사(inspection)에 착수했다. 다른 회원국들이 사설 경고장 또는 시민단체 가처분 중심으로 움직이는 것과 대조적으로, 스웨덴은 행정 규제기관이 직접 점검에 나섰다. 본 가이드의 "분기별 자동 스캔 + 보고서 보관(섹션 10의 분기별 검증 일정)"이 규제 검사 대응 시 즉시 제출 가능한 형태로 준비되어야 한다.

> **사례 4 -- 네덜란드 ACM, 2026년 봄 우선 감사 대상 통지**
> 네덜란드 소비자시장청(ACM)은 EAA 보고서를 누락하거나 불완전 제출한 사업자를 2026년 봄 우선 감사 대상으로 지정한다고 사전 공지했다. 본 가이드의 분기별 보고서 + 사용자 테스트 결과 + 수정 계획 트래킹이 ACM 감사 대응의 사실상 표준 양식이 된다.

> **사례 5 -- 아일랜드, 형사처벌 가능 유일 회원국**
> 아일랜드는 EAA 위반을 형사 책임(criminal liability)까지 물을 수 있는 유일하게 확인된 EU 회원국이다. 시정 통지(notice to comply) 후에도 미준수 시 최대 60,000 EUR 벌금 또는 18개월 징역형이 가능하다. 아일랜드 시장 진출 서비스는 본 가이드의 production AAA 목표를 반드시 적용한다.

### 12.2 미국 ADA Title II 시행 연기 (2026.04)

> **사례 6 -- 2026.04.24 시한 직전 1년 연장(IFR)**
> 원래 2026.04.24가 인구 5만 이상 미국 주/지방정부 대상 ADA Title II Web Accessibility Rule 시한이었다. 그러나 시한 4일 전인 2026.04.20, 법무부(DOJ)가 임시 최종 규칙(Interim Final Rule)을 발표하여 시한을 1년 연장했다. **대형 자치단체는 2027.04.26**, **소형 자치단체와 특수구는 2028.04.26**으로 각각 미뤄졌다. 본 가이드 머리말의 "2026.04.26 시한"은 이 IFR로 인해 1년 후로 이동했음을 운영 시점에 반영해야 한다. 단, ADA Title III(민간 사업자)에는 영향이 없으며 2025년 한 해에만 연방 법원에 3,117건의 ADA 웹 접근성 소송이 제기되어(전년 대비 +27%) 민간 부문의 압박은 오히려 강해지고 있다.

### 12.3 axe DevTools/MCP 활용 사례

> **사례 7 -- axe MCP Server를 통한 IDE 자동 수정 표준화 (2026)**
> Deque가 axe MCP Server를 axe DevTools for Web 번들에 무료 포함시키면서, GitHub Copilot, Cursor, Claude Code, Windsurf, VS Code 사용자가 IDE 내부에서 접근성 위반을 1-클릭으로 수정할 수 있는 워크플로우가 보편화되고 있다. 본 가이드의 `a11y-auto-fix.ts`(섹션 5.2)는 PR 단위 일괄 수정 제안에 적합하고, axe MCP는 코드 작성 시점의 인라인 수정에 적합하다. 두 도구를 결합한 4단계 방어선(IDE -> Linter -> Lint Action -> 런타임 axe-core)이 2026년 사실상의 표준 구성이다.

> **사례 8 -- axe Linter VS Code 확장 600k+ 다운로드 (2025.10 기준)**
> Deque가 무료로 배포하는 axe Accessibility Linter for VS Code는 2025년 10월 기준 60만 다운로드를 돌파했다. 본 가이드는 이 무료 확장을 모든 신규 멤버의 Codespace `devcontainer.json`에 기본 포함시키는 것을 권장한다([17. 신규 입사자 온보딩 가이드](./17_신규_입사자_온보딩_가이드.md) 섹션 11의 듀얼 모드 devcontainer 예시 참고).

### 12.4 EAA/ADA 운영 체크리스트 (2026 추가)

본 가이드 섹션 10의 기존 체크리스트에 더해, 2026년 규제 환경에서 신규로 점검할 항목:
...
- [ ] ADA Title II 적용 미국 자치단체의 경우, 2027.04.26 새 시한에 맞춰 단계적 도입 로드맵이 있는가
- [ ] axe DevTools Linter `axe-linter.yml`이 staging WCAG 2.2 AA와 일치하는가
- [ ] axe MCP Server가 IDE에 연결되어 있고, 모든 AI 에이전트(Claude Code/Cursor/Copilot)가 동일 룰셋을 보는가
...
| 자동 검증 (정적) | [axe DevTools Linter 4.x](https://www.deque.com/axe/devtools/linter/) (44개 룰, React/Vue/Angular/HTML/Markdown) |
| AI 자동 수정 | [axe MCP Server (axe DevTools for Web 번들)](https://www.deque.com/axe/mcp-server/) (Claude Code/Cursor/Copilot/Windsurf 호환) |
...
| 규제 (US) | [ADA Title II Web Accessibility Rule (DOJ IFR로 2027.04.26 연장)](https://www.ada.gov/resources/2024-03-08-web-rule/) |
| 규제 (US, 2026 연장) | [Federal Register: ADA Title II Compliance Date Extension (2026.04.20)](https://www.federalregister.gov/documents/2026/04/20/2026-07663) |
