...
| **핵심 테마** | Vite 8 (Rolldown 1.0 default), Core Web Vitals 2026, React Compiler 1.0, View Transitions L2, Speculation Rules | **Update** | 2026.05 |

---

> **"성능은 기능이 아니라 인프라다. 2026년의 프론트엔드는 툴체인이 대신 최적화하고, 개발자는 지표를 관리한다."**
> 본 가이드는 Vite 8(Rolldown 1.0)과 React Compiler 1.0, 2026년 3월 갱신된 Core Web Vitals 임계값에 맞춘 최신 성능 최적화 전략을 다룹니다.

---

## 1. 빌드 도구: Vite 8 & Rolldown 1.0

2026년 기준 **Vite 8**가 정식 출시되어 Rust 기반 번들러인 **Rolldown 1.0**을 기본(default) 번들러로 사용합니다. 기존 `rolldown-vite` 별도 패키지는 2026년 3월에 아카이브되었고, 이제 `vite` 단일 패키지로 빌드 시 **10-30배** 빠른 속도를 얻을 수 있습니다(플러그인 API는 Rollup 호환 유지).
...
Vite 6 이후 PostCSS 대신 **Lightning CSS**를 사용해 CSS 컴파일 및 미니피케이션 속도를 10배 이상 끌어올렸고, Vite 8에서는 Rolldown 1.0과 결합되어 빌드 파이프라인 전체가 Rust 네이티브로 동작합니다.
...
**React Compiler v1.0**이 2025년 10월 7일 정식 출시되어 stable 단계로 진입했습니다. Meta의 대형 앱에서 검증된 결과, 초기 로드와 페이지 전환은 최대 12% 빨라졌고 특정 인터랙션은 2.5배 이상 빨라졌습니다. 2026년 5월 기준 안정 버전은 React 19.2.6이며, Compiler는 React Native와도 호환됩니다.
...
## 3. 핵심 사용자 지표: 2026 Core Web Vitals

2026년 3월 Google Core Update로 임계값이 갱신되었습니다. **INP가 LCP·CLS와 동등한 핵심 랭킹 시그널**로 격상되었으며, LCP의 "Good" 기준이 더 엄격해졌습니다. CrUX 데이터 기준 2026년 초 전 세계 웹사이트의 약 43%가 INP 200ms 기준을 통과하지 못하고 있어, INP가 가장 흔히 실패하는 지표입니다.

### 3.0 2026 임계값 표

| 지표 | Good | Needs Improvement | Poor | 비고 |
| :--- | :--- | :--- | :--- | :--- |
| **LCP** | **≤ 2.0s** (2026 변경) | 2.0s ~ 2.5s | > 2.5s | 기존 2.5s에서 2.0s로 강화 |
...
> **권장 운영 기준**: 75퍼센타일(p75) 모바일 사용자 기준으로 LCP 2.0s, INP 200ms, CLS 0.1을 동시에 충족해야 "Pass" 됩니다. 단일 지표라도 실패하면 SEO 영향이 발생합니다.
...
#### Speculation Rules + View Transitions Level 2 결합 (2026 권장)

2026년 기준 **View Transitions Level 2(cross-document)** 가 Chrome·Edge·Safari 18.2 모두에서 안정 지원됩니다. Speculation Rules의 prerender와 `@view-transition` 규칙을 결합하면 MPA에서도 SPA 수준의 부드러운 전환을 얻을 수 있어, "SPA 없이도 즉각적 네비게이션"이 가능해졌습니다.
...
module.exports = {
  ci: {
    ...
    assert: {
      assertions: {
        // Core Web Vitals 임계값 설정 (2026 갱신 기준)
        'categories:performance': ['error', { minScore: 0.9 }],
        'categories:accessibility': ['warn', { minScore: 0.95 }],
        // 개별 지표 기준 — LCP는 2026년부터 2.0s 기준
...
const PERFORMANCE_BUDGET = {
  // Core Web Vitals (2026년 3월 코어 업데이트 반영)
  LCP: 2000,    // ms — Good: ≤2.0s (2025년 2.5s에서 강화)
...
| 잘못된 지표 | 올바른 지표 | 이유 |
| :--- | :--- | :--- |
| 페이지 로드 이벤트 시간 | LCP (≤ 2.0s, 2026 기준) | 실제 사용자가 콘텐츠를 보는 시점 — 2026년부터 2.0s로 강화 |
| FID (First Input Delay) | INP (Interaction to Next Paint) | FID는 2024년 폐기, 2026 코어 업데이트에서 INP가 핵심 시그널 |
...
### 빌드 & 번들
- [ ] Vite 8(Rolldown 1.0 default)로 업그레이드했고, deprecated `rolldown-vite` 별도 패키지를 제거했나요?
...
### Core Web Vitals (2026 갱신 기준)
- [ ] LCP가 **2.0s 이하**인가요? (2025년 2.5s에서 강화된 신규 임계값)
- [ ] LCP 대상 이미지에 `fetchpriority="high"`가 적용되었나요?
- [ ] INP p75가 200ms 이하인가요? (2026년부터 LCP·CLS와 동등한 핵심 랭킹 시그널)
