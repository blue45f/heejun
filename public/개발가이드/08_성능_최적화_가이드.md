...
| **핵심 테마** | Vite 8 (Rolldown/Oxc), Core Web Vitals, React Compiler 1.0, View Transitions, Speculation Rules | **Update** | 2026.05 |
...
2026년 기준 **Vite 8**은 Rolldown과 Oxc 기반 도구 체인으로 전환되었습니다. `rolldown-vite`로 사전 검증하던 팀은 Vite 8 마이그레이션 가이드에 맞춰 `vite` 단일 패키지 기준으로 플러그인 호환성, 청크 결과, CSS 처리 결과를 다시 검증합니다. 성능 수치는 프로젝트 구조와 플러그인에 따라 달라지므로, 마이그레이션 PR에는 빌드 시간/번들 크기/런타임 지표 비교를 반드시 첨부합니다.
...
**React Compiler v1.0**이 2025년 10월 7일 정식 출시되어 stable 단계로 진입했습니다. 2026년 5월 기준 운영 baseline은 React 19.2 라인이며, RSC를 사용하는 경우 `react-server-dom-*` 패키지 19.2.4 이상 보안 패치를 별도 확인합니다. Compiler 도입 여부는 lint 결과, hydration 안정성, 실제 interaction trace 개선으로 판단합니다.
...
- **Field-first 성능 운영**: lab 지표와 함께 RUM p75, INP 회귀율, CLS 이벤트 빈도를 기준 게이트로 고정합니다.
- **번들/렌더링 계약**: JS/CSS 캐시 전략, 코드 분할, React Compiler 준수 범위를 문서화하고 예산 위반 시 배포 중단 룰을 둡니다.
- **입력 지연 완화**: 입력/스크롤 처리에서 scheduler/yield, worker 분리, 이미지 우선순위 정책을 시나리오별로 계량화합니다.
- **성능 감사 자동화**: 성능 스냅샷을 주간/월간 비교해 회귀 지점을 자동 분류하고 담당자에게 알림합니다.
...
- Field-first 운영: p75 및 인풋 지표를 CI 예산과 결합해 성능 회귀를 릴리즈 게이트에서 즉시 차단합니다.
- 코드 분할/캐시 재평가: chunk 크기, prefetch, priority hint를 월간으로 재조정해 장기 회귀를 줄입니다.
- 입력 응답성 우선화: Interaction 중심 KPI를 운영하고 개선치가 없으면 기본 정책으로 롤백합니다.
- 저사양 장치 대응: frame drop, 메모리, 배터리 지표를 장치군별로 분리 관측합니다.
- 컴파일 비용 균형: 번들 최적화 설정 변경은 빌드 시간 증가분과 런타임 개선분을 정량 비교 후 반영합니다.

- RUM/INP/LCP 회귀를 기능 릴리즈 단위가 아닌 사용자 흐름 단위로 묶어 분석해 성능 의사결정의 오해를 줄입니다.
- 지표 개선이 작은 구간에서만 나타나는 변화는 프로필 분할(로우엔드/와이파이/노약기기)별로 확인해 실제 체감 개선 여부를 재정의합니다.
- 실험 기능은 `web-vitals/attribution` 원인 분해를 요구해, 단일 숫자 개선으로 오판하지 않도록 합니다.
- 성능 개선 실험은 배포 단계별로 독립 예산을 두어, 개선 효과 미달 기능은 다음 배포에서 축소 또는 철회합니다.
- 사용자 체감 성능은 장치군·회선군·지역군별로 분리해 측정하고, 한 항목이라도 고위험이면 자동 롤백 트리거를 실행합니다.
- 성능 회귀 탐지는 평균값보다 상위 구간 p95/p99 회귀를 함께 기준으로 삼아 일부 사용자군 피해를 선제 차단합니다.
- 기능 정체 구간은 번들/렌더 경로 분석으로 병목을 정량 비교해 다음 릴리즈의 투자 우선순위를 재조정합니다.
- 성능 비용 대비 개선 이득이 낮은 기능은 우선순위를 낮춰 다음 분기까지 보류 후보로 전환합니다.
- 사용자 체감 품질 지표는 회귀 탐지 이후 바로 롤백 여부를 결정하는 최우선 신호로 유지합니다.
...
- **성능 예산 집행**: 회귀 방지 예산을 우선 영역별로 분배하고 초과 시 새 기능 확장을 제한합니다.
- **저사양 우선 검증**: 저사양군에서 임계치 초과 시 기본 경험으로 fallback 합니다.
...
- [ ] **성과 지표 합치**: p75/p99 및 저사양군 체감 지표 변화가 허용범위 내인지 확인한다.
- [ ] **배포 전후 비교**: 최적화 변경은 이전 버전과 번들/성능 벤치마크 차이를 기록한다.