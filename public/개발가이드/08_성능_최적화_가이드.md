...
> **일상 비유**: 성능 최적화는 막힌 도로를 통째로 새로 까는 게 아니라, 교통 흐름을 측정해 막힌 교차로 한 곳만 먼저 정비하는 일에 가깝습니다. 어디가 막혔는지 모르면 새 도로를 깔아도 차는 그대로 막힙니다.
...
```mermaid
flowchart LR
  M["측정<br/>RUM + Lighthouse"] --> D["진단<br/>병목 구간 식별"]
  D --> A["조치<br/>이미지/번들/렌더"]
  A --> V["검증<br/>before-after 비교"]
  V -->|회귀 없음| R["배포"]
  V -->|회귀| D
  R --> O["관측<br/>p75 회귀 알림"]
  O --> M
...
> 이 루프를 한 번 도는 것이 "최적화 1 라운드"입니다. 측정 없이 조치부터 시작하면 다음 라운드를 추측에 맡기게 됩니다.
...
- **시작 추천**: LCP/INP/CLS 임계치 기준부터 잡고 우선 모니터링 지표를 설정하세요.
- **안정 추천**: 매 페이지별 병목 1개씩만 먼저 고치며, 캐시/번들/이미지 세 파트 중 한 가지만 집중합니다.
- **운영 추천**: 성능 예산 회귀는 릴리즈 전 필수 게이트로 두고 무관한 리팩터만으로는 승인하지 않습니다.

## 추천 항목 고도화 체크

- `첫 적용` — Core Web Vitals와 bundle budget 중 하나를 실제 PR이나 운영 이슈에 붙이고, 변경 전 기준을 먼저 적는다.
- `증거 정리` — RUM p75, Lighthouse trace, bundle analyzer 결과를 같은 작업 기록에 남긴다.
- `재점검` — LCP, INP, CLS, JS KB, long task 수가 나아졌는지 30일 안에 확인하고 기준을 유지, 수정, 폐기 중 하나로 판정한다.

## 추천 항목 실행 기록 템플릿

- `작업` : Core Web Vitals와 bundle budget 적용 범위를 어느 화면, 패키지, 문서에 둘지 적는다.
- `증거` : RUM p75, Lighthouse trace, bundle analyzer 결과 중 실제로 남긴 항목만 링크한다.
- `판정` : 유지/수정/폐기 중 하나와 이유를 한 문장으로 남긴다.
- `다음 점검` : LCP, INP, CLS, JS KB, long task 수를 다시 볼 날짜와 담당자를 지정한다.
...
> **왜 중요한가**: 수동 `memo`/`useMemo`/`useCallback`은 코드 복잡도를 키우는 반면, 휴먼이 의존성 배열을 잘못 쓰면 오히려 버그를 만듭니다. Compiler는 이 부담을 컴파일러로 옮깁니다.
>
> **일상 비유**: 수동 메모이제이션이 "주방에서 셰프가 매번 음식 재사용 가능 여부를 직접 판단"하는 일이었다면, Compiler는 "오븐이 자동으로 식재료 신선도를 체크"하는 자동화입니다.

### React Compiler 도입 흐름

```mermaid
flowchart LR
  Code["기존 코드<br/>memo/useMemo/useCallback"] --> Lint["eslint-plugin-react-hooks<br/>compiler-powered rules"]
  Lint -->|위반 0| Comp["React Compiler 빌드"]
  Lint -->|위반 있음| Fix["Rules of React 수정"]
  Fix --> Lint
  Comp --> Bench["성능 비교<br/>interaction, hydration, render count"]
  Bench -->|개선| Adopt["수동 memo 제거 마이그레이션"]
  Bench -->|회귀| OptOut["해당 파일 opt-out"]
  Adopt --> Done["프로덕션 적용"]
  OptOut --> Done
```
...
> **왜 중요한가**: Core Web Vitals는 검색 순위와 사용자 이탈 모두에 영향을 주는 핵심 표준 사용자 경험 지표입니다. 임계값을 모르면 "느린데 왜 느린지 설명할 수 없는" 상태에 빠집니다.
>
> **일상 비유**: LCP는 식당에서 메인 요리가 나오는 시간, INP는 종업원을 불렀을 때 응답하는 속도, CLS는 식기가 움직여 음식을 흘릴 위험에 비유할 수 있습니다. 셋 다 별개의 만족도를 만듭니다.
...
### Core Web Vitals 임계 구간

```mermaid
flowchart LR
  subgraph LCP["LCP - 가장 큰 콘텐츠 표시 시간"]
    L1["Good<br/>≤ 2.5s"] --- L2["Needs Improvement<br/>2.5s ~ 4.0s"] --- L3["Poor<br/>> 4.0s"]
  end
  subgraph INP["INP - 입력 후 다음 페인트"]
    I1["Good<br/>≤ 200ms"] --- I2["Needs Improvement<br/>200ms ~ 500ms"] --- I3["Poor<br/>> 500ms"]
  end
  subgraph CLS["CLS - 누적 레이아웃 이동"]
    C1["Good<br/>≤ 0.1"] --- C2["Needs Improvement<br/>0.1 ~ 0.25"] --- C3["Poor<br/>> 0.25"]
  end
```

### LCP 분해: 어디서 시간이 새는가

LCP는 단일 숫자가 아니라 4개의 하위 구간으로 분해됩니다. 어느 구간이 큰지 알아야 어떤 조치가 효과적인지 판단할 수 있습니다.

> **일상 비유**: LCP는 택배 도착 시간 같습니다. (1) 주문 접수 → (2) 창고 픽업 → (3) 배송 트럭 이동 → (4) 문 앞 전달. 어느 단계가 느린지 알아야 "트럭을 더 빠르게 할지, 창고를 늘릴지" 결정할 수 있습니다.

```mermaid
flowchart LR
  A["사용자 요청"] --> B["TTFB<br/>(서버 응답)"]
  B --> C["Resource Load Delay<br/>(LCP 자원 발견까지)"]
  C --> D["Resource Load Time<br/>(이미지/폰트 다운로드)"]
  D --> E["Element Render Delay<br/>(렌더 차단 제거)"]
  E --> F["LCP 발생"]
  B -.개선.-> B1["SSR/CDN/캐시"]
  C -.개선.-> C1["preload, fetchpriority"]
  D -.개선.-> D1["AVIF/WebP, 크기 최적화"]
  E -.개선.-> E1["CSS 차단 해제, hydration 분할"]
```

### INP 발생 흐름

INP는 단일 인터랙션의 응답 지연을 측정합니다. 사용자가 클릭한 순간부터 화면이 시각적으로 반응한 순간까지의 시간을 합산합니다.

```mermaid
sequenceDiagram
  participant U as 사용자
  participant Br as 브라우저(메인 스레드)
  participant App as App 코드
  U->>Br: 클릭/탭/키 입력
  Note over Br: Input Delay<br/>(스레드가 다른 작업 처리 중)
  Br->>App: 이벤트 핸들러 호출
  Note over App: Processing<br/>(state 업데이트, 계산)
  App->>Br: 렌더 요청
  Note over Br: Presentation Delay<br/>(레이아웃 + 페인트)
  Br-->>U: 화면 갱신
  Note over U,Br: INP = 입력 ~ 시각적 반응
```

### Web Vitals 조치 결정 트리

> **왜 트리가 필요한가**: "LCP가 느리다"는 진술 자체로는 조치가 없습니다. 어느 구간이 느린지에 따라 완전히 다른 작업이 필요합니다.

```mermaid
flowchart TD
  S["Web Vitals 회귀 감지"] --> Q1{"어느 지표?"}
  Q1 -->|LCP| L["LCP 분해 확인"]
  Q1 -->|INP| I["interaction target 확인"]
  Q1 -->|CLS| C["shifted element 확인"]
  L --> LQ{"가장 큰 구간?"}
  LQ -->|TTFB| TA["서버/CDN/캐시 점검"]
  LQ -->|Load Delay| TB["preload + fetchpriority"]
  LQ -->|Load Time| TC["이미지 포맷/크기 최적화"]
  LQ -->|Render Delay| TD["CSS 차단/hydration 분리"]
  I --> IQ{"long task 여부"}
  IQ -->|있음| IA["startTransition/worker"]
  IQ -->|없음| IB["handler 코드 경량화"]
  C --> CQ{"shift 원인?"}
  CQ -->|이미지| CA["width/height 명시"]
  CQ -->|폰트| CB["font-display + size-adjust"]
  CQ -->|광고/배너| CC["고정 placeholder 영역 확보"]
```
...
function SearchPage() {
  ...
  return (
    <div>
      <input value={query} onChange={handleSearch} placeholder="검색어를 입력하세요..." />
      {/* 전환 중일 때 시각적 피드백 제공 */}
      {isPending && <Spinner />}
...
> **왜 중요한가**: 코드를 최적화하는 데는 한계가 있지만, "사용자가 클릭하기 전에 미리 준비"하면 체감 속도를 0초에 가깝게 줄일 수 있습니다. 이 섹션은 코드를 줄이는 게 아니라 "타이밍"을 옮기는 기법입니다.
>
> **일상 비유**: 카페에서 단골 손님이 늘 같은 메뉴를 시킨다면, 손님이 들어오는 순간 음료를 미리 만들어 둡니다. Speculation Rules는 브라우저에게 "이 손님이 곧 시킬 메뉴(URL)"를 알려주는 셈입니다.

### 4.0 사전 최적화 기법 비교

```mermaid
flowchart LR
  subgraph timeline["사용자 행동 타임라인"]
    T1["페이지 로드"] --> T2["hover/스크롤"] --> T3["클릭/탭"] --> T4["다음 페이지"]
  end
  T1 -.preload.-> P1["LCP 자원 우선 다운"]
  T1 -.prefetch.-> P2["다음 페이지 자원 캐시"]
  T2 -.moderate prerender.-> P3["다음 페이지 미리 렌더"]
  T3 -.conservative prerender.-> P4["pointerdown 시점 렌더"]
  T4 -.View Transition.-> P5["부드러운 전환"]
```
...
> **왜 중요한가**: 사용자가 첫 화면을 보기까지 다운로드 → 파싱 → 실행 단계를 거치는데, 이 전체에 번들 크기가 1차 영향을 줍니다. JS는 단순히 다운로드만 비용이 아니라 파싱·컴파일·실행까지 모두 비용입니다.
>
> **일상 비유**: 번들은 손님이 입장 전에 받는 안내서 두께입니다. 안내서가 두꺼울수록 손님이 본문(앱)을 시작하는 시점이 늦어집니다. 그렇다고 안내서를 잘게 찢어 100장으로 나누면 들고 다니기 더 불편하니, 묶음 단위에도 적정선이 있습니다.

### 5.0 번들 크기와 사용자 영향

```mermaid
flowchart LR
  B["번들 크기"] --> D["Download Time<br/>(네트워크 의존)"]
  B --> P["Parse Time<br/>(엔진 의존)"]
  B --> C["Compile Time<br/>(엔진 의존)"]
  B --> E["Execute Time<br/>(실행)"]
  D --> FCP["FCP/LCP 지연"]
  P --> FCP
  C --> FCP
  E --> INP["INP 악화<br/>(메인 스레드 점유)"]
```

| 번들 영향 구간 | 1차 측정                           | 1차 조치                          |
| :------------- | :--------------------------------- | :-------------------------------- |
| Download       | Network panel, 3G/4G 시뮬레이션    | tree-shaking, 동적 import, brotli |
| Parse/Compile  | Performance panel "Compile script" | 작은 청크 분리, modern target     |
| Execute        | Long Task, main thread block       | startTransition, worker 분리      |

### Performance Budget 결정 트리

> **왜 트리가 필요한가**: 새 라이브러리 도입 PR이 올라올 때마다 즉석에서 판단하면 일관성이 흔들립니다. 사전에 합의된 결정 트리를 두면 리뷰가 빨라집니다.

```mermaid
flowchart TD
  N["새 의존성 추가 PR"] --> Q1{"gzip 추가 크기?"}
  Q1 -->|< 5KB| OK1["승인 가능"]
  Q1 -->|5KB ~ 20KB| Q2{"동적 import 가능?"}
  Q1 -->|> 20KB| Q3{"대체 라이브러리?"}
  Q2 -->|가능| OK2["lazy 적용 후 승인"]
  Q2 -->|불가| Q3
  Q3 -->|있음| ALT["대체 라이브러리 검토"]
  Q3 -->|없음| Q4{"핵심 경로?"}
  Q4 -->|예| NEEDS["budget 재협상 + ADR"]
  Q4 -->|아니오| LAZY["lazy 강제 + budget 분리"]
```
...
> **왜 중요한가**: DOM 노드 개수는 렌더링 비용 외에도 메모리, 이벤트 처리, 레이아웃 계산에 모두 영향을 줍니다. 1만 개 항목을 다 렌더링하면 브라우저가 멈춥니다.
>
> **일상 비유**: 도서관에서 모든 책을 한 줄로 펼쳐 놓지 않고 책꽂이에서 사용자가 보는 칸의 책만 꺼내는 것과 같습니다. 사용자에게는 전체가 있는 것처럼 보이지만, 실제로 꺼낸 책은 몇 권뿐입니다.

### 가상 스크롤 동작 원리

```mermaid
flowchart TD
  subgraph data["전체 데이터 (10,000건)"]
    D1["item 0"] ~~~ D2["item 1"] ~~~ D3["..."] ~~~ D4["item 9999"]
  end
  subgraph dom["실제 DOM (보이는 영역 + overscan)"]
    V1["item 142"] --> V2["item 143"] --> V3["..."] --> V4["item 158"]
  end
  data -->|virtualizer.getVirtualItems| dom
  dom -->|스크롤 이벤트| RC["보이는 인덱스 재계산"]
  RC -->|getVirtualItems| dom
```
...
> **왜 중요한가**: lab 점수(Lighthouse)는 재현 가능하지만 실제 사용자 환경과 다릅니다. field 데이터(RUM)는 진짜이지만 노이즈가 많습니다. 두 데이터를 같은 시점에 함께 보지 않으면 잘못된 결론에 도달합니다.
>
> **일상 비유**: 병원에서 의사가 환자의 X-ray(lab)만 보고 치료를 결정하지 않고, 실제 증상 호소(field)를 함께 듣는 것과 같습니다. 둘을 교차 검증해야 합니다.

### 8.0 lab vs field 데이터 흐름

```mermaid
flowchart LR
  subgraph CI["배포 전: Lab"]
    LH["Lighthouse"] --> LR["lab score, trace"]
    LR --> Gate["PR gate"]
  end
  subgraph Prod["배포 후: Field"]
    RUM["web-vitals + RUM"] --> RP["p75 by route/device"]
    RP --> Alert["회귀 알림"]
  end
  Gate -->|배포| Prod
  Alert -->|회귀 발견| Inv["원인 추적"]
  Inv -->|재현| LH
```
...
const SimpleButton = memo(({ label, onClick }) => <button onClick={onClick}>{label}</button>)
...
- `P1(7일 내)` — Core Web Vitals와 bundle budget 중 하나를 작은 변경 1건에 적용하고 증거(RUM p75)를 남긴다.
- `P2(30일 내)` — 성능 예산 기준을 팀 템플릿, 체크리스트, CI 중 한 곳에 고정한다.
- `P3(90일 내)` — LCP, INP, CLS, JS KB, long task 수 추이를 보고 기준을 유지할지 조정할지 결정한다.
- `완료 기준` — 성능 오너가 증거와 철회 조건을 확인했다는 기록을 남긴다.

## 추천 항목 실행 체크리스트

- [ ] `1단계(7일)` : Core Web Vitals와 bundle budget 적용 대상을 1개로 좁힌다.
- [ ] `2단계(30일)` : 증거(RUM p75, Lighthouse trace, bundle analyzer 결과)를 PR, ADR, 회고 중 한 곳에 연결한다.
- [ ] `3단계(60일)` : LCP, INP, CLS, JS KB, long task 수가 기준 안에 들어왔는지 확인한다.
- [ ] `문제 대응` : 미달성 사유와 다음 조치, 중단 여부를 같은 기록에 남긴다.

## 추천 항목 실행 운영 규칙

- `실행 게이트` : lab 점수보다 실제 사용자 p75가 어디서 나빠졌는지 확인한다.
- `승인 체계` : 성능 오너가 영향 범위와 rollback 담당자를 적용 전에 확인한다.
- `재개 조건` : field와 lab 지표가 같은 방향이면 최적화를 유지한다.
- `정지 조건` : 한 지표 개선이 다른 핵심 지표를 악화시키면 rollback한다.
- `리스크 점수` : 트래픽 비중, p75 악화 폭, third-party 영향으로 산정한다.
- `리더 승인자` : 성능 리드가 최종 승인 책임을 맡는다.
- `승인 역할` : 성능 예산 작성자, 검토자, 운영 확인자를 분리해 기록한다.
- `재평가 주기` : 릴리스마다 Web Vitals 추이를 확인한다.
