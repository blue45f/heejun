...
| 분류 | 품질 & 성능 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [04. 아키텍처](./04_아키텍처_설계_패턴.md), [10. 인프라](./10_인프라_및_AWS_CDK_가이드.md), [11. CI/CD](./11_CICD_파이프라인_표준.md) | **AI 도구** | Vitest 4, Playwright Agents, Claude Code |
...
Vite 생태계에 최적화된 **Vitest 4**와 네트워크 레이어 모킹인 **MSW 2.x**를 결합하여 신속하고 정확한 테스트 환경을 구축합니다. 2025년 말 출시된 **Vitest 4**는 **Browser Mode가 안정화(Stable)** 되었고, `toMatchScreenshot()` 기반의 **시각 회귀 테스트**, **Playwright Trace 통합**이 내장되어 Jest 대비 약 20배 빠른 실행 속도를 제공합니다. MSW 2.x는 REST/GraphQL에 더해 **WebSocket 모킹**까지 1급으로 지원합니다.
    ...
    projects: [
      ...
      },
      {
        ...
        test: {
          ...
          browser: {
            enabled: true,
            provider: 'playwright',     // 'playwright' 또는 'webdriverio'
            headless: true,
            instances: [
              { browser: 'chromium' },
              { browser: 'firefox' },
...
## 2. 시나리오 테스트: Playwright 1.56+ & Test Agents

독립적으로 구성된 Preview 환경을 대상으로 실제 런타임 환경에서의 비즈니스 흐름을 검증합니다. **Playwright 1.50부터 구조화된 trace 데이터가 LLM 입력으로 직접 전달**되도록 개선되었고, **1.56부터 공식 Test Agents(Planner / Generator / Healer)** 가 제공되어 AI 기반 테스트 생성·자가 복구가 표준이 되었습니다.
...
AI(Claude Code, Playwright Test Agents 등)를 활용하여 엣지 케이스를 탐지하고 테스트 코드를 자동 생성하여 커버리지를 효율적으로 높입니다. 2026년 기준 QA 리더 76%가 AI 기반 테스트 생성을 표준 또는 파일럿으로 운영하고 있다는 보고가 있을 정도로 보편화되었습니다.

### 3.0 Playwright Test Agents (1.56+)

Playwright 1.56부터 VS Code에 통합된 세 가지 공식 에이전트가 제공됩니다. MCP(Model Context Protocol) 서버를 통해 접근성 트리 스냅샷 기반으로 동작하기 때문에 스크린샷 기반 에이전트보다 훨씬 안정적입니다.
...
```bash
# .github/workflows 또는 로컬에서 Healer를 CI flaky 테스트 복구에 활용
npx playwright test --reporter=line
npx playwright agent heal --last-failures
```
...
#### Claude Code에 테스트 생성 요청하는 실전 프롬프트

```
다음 컴포넌트에 대한 Vitest + Testing Library 테스트를 작성해줘:
...
테스트 코드 자체의 유효성을 검증하기 위해 코드를 의도적으로 변형시킨 후 테스트가 실패하는지 확인하여 테스트의 견고함을 평가합니다. 2026년 기준 GitHub Actions에서 80% 임계값을 강제한 팀들에서 거짓 음성(false negative)이 약 60% 감소했다는 사례가 보고되고 있습니다.
...
jobs:
  ...
  unit-integration:
    ...
    steps:
      ...
      # Vitest 병렬 실행 (샤딩)
      - name: 테스트 실행
        run: pnpm vitest run --coverage --reporter=github-actions
...
- [ ] GitHub Actions에서 단위/통합/E2E 테스트가 PR마다 자동 실행되나요?
