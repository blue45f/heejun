...
| 브라우저 모드/시각 회귀 | 테스트 러너 공식 릴리스 | screenshot baseline, trace artifact | UI incident, layout regression | false positive가 높으면 범위 축소 |
| AI 테스트 생성 | [18. AI 워크플로우](./18_AI_개발_워크플로우_종합.md) 정책 | generated test가 실제 결함에서 실패 | 누락 케이스 감소 | 재현/검증 없는 테스트는 merge 금지 |
...
독립적으로 구성된 Preview 환경을 대상으로 실제 런타임 환경에서의 비즈니스 흐름을 검증합니다. **Playwright 1.50부터 구조화된 trace 데이터가 LLM 입력으로 직접 전달**되도록 개선되었고, **1.56부터 공식 Test Agents(Planner / Generator / Healer)** 가 제공되어 AI 기반 테스트 생성·자가 복구를 제한적으로 활용할 수 있습니다.
...
AI 보조 도구를 활용하여 엣지 케이스를 탐지하고 테스트 코드를 자동 생성하여 커버리지를 효율적으로 높입니다. 2026년 기준 QA 리더 76%가 AI 기반 테스트 생성을 표준 또는 파일럿으로 운영하고 있다는 보고가 있을 정도로 보편화되었습니다.
...
```bash
# .ci/workflows 또는 로컬에서 Healer를 CI flaky 테스트 복구에 활용
