# 07. 테스팅 가이드 (2026 Edition)

| 분류 | 품질 & 성능 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [04. 아키텍처](./04_아키텍처_설계_패턴.md), [10. 인프라](./10_인프라_IaC_가이드.md), [11. CI/CD](./11_CICD_파이프라인_표준.md) | **도구 원칙** | 벤더 중립 |
| **핵심 테마** | Vitest Browser Mode 안정화, Playwright Agents(Planner/Generator/Healer), AI 테스트 생성, 시각 회귀, 뮤테이션 테스팅 | **Update** | 2026.05 |
...
### Vitest 4 Browser Mode (Stable, 2026)
...
독립적으로 구성된 Preview 환경을 대상으로 실제 런타임 환경에서의 비즈니스 흐름을 검증합니다. 2026-05 기준 최신 Playwright 문서는 1.60 라인을 기준으로 보되, **Test Agents(Planner / Generator / Healer)** 는 1.56에서 도입된 공식 agent definition 흐름으로 관리합니다. AI 기반 테스트 생성과 자가 복구는 trace, seed test, 사람 리뷰가 있을 때만 제한적으로 사용합니다.
...
테스트 코드 자체의 유효성을 검증하기 위해 코드를 의도적으로 변형시킨 후 테스트가 실패하는지 확인하여 테스트의 견고함을 평가합니다. 2026년 기준 CI actions에서 80% 임계값을 강제한 팀들에서 거짓 음성(false negative)이 약 60% 감소했다는 사례가 보고되고 있습니다.
...
- **테스트 신뢰도 강화**: 단위/컴포넌트/시나리오/시각 회귀 테스트를 의도별 회귀 영향도×비용으로 우선순위화합니다.
- **브라우저 현실 테스트**: Playwright matrix에 주요 브라우저/디바이스 + 네트워크 상태를 반영하고, AI 생성 테스트 케이스는 별도 샘플링 검수를 추가합니다.
- **성능·접근성 동시 게이트**: 성능 회귀 테스트와 접근성 자동 검사가 하나의 파이프라인 artifact로 묶여야 합니다.
- **에이전트 기반 테스트 운영**: 테스트 에이전트 사용 시 flaky 제어, 비결정성 허용치, 리트라이 정책을 문서화해 재현성 리스크를 낮춥니다.
...
- 테스트 피라미드 재정의: 핵심 비즈니스 시나리오를 계약 테스트로 끌어올리고 유의미 지표 기반으로 우선순위를 조정합니다.
- AI 테스트 산출물 검증: AI 생성 테스트는 샘플링 리뷰 후 CI 승인 후보로 이동하고 오탐률이 높으면 제외합니다.
- Playwright 매트릭스 고정: 디바이스·네트워크 조건별 smoke 템플릿을 월간으로 적용해 실패율 추이를 추적합니다.
- flaky 관리: flaky 테스트는 오너 라벨과 재현성 분석 임계치를 두고 개선 주기를 고정합니다.
- 커버리지 보정: 라인 커버리지보다 비즈니스 시나리오 회귀 지표를 우선 지표로 사용합니다.

- 테스트 전략은 기능 정확성보다 운영 민감 구간(rollback, 재시도, 동시성) 커버리지를 더 우선해, 장애율 상승 구간을 선제적으로 차단합니다.
- Playwright/계측 테스트는 브라우저/기기 다변성에서 실패 패턴을 축적해 다음 스프린트의 우선순위를 자동 보정합니다.
- AI·자동화가 생성한 테스트는 반드시 사람이 재현 가능한 최소한의 실패 근거와 함께 리뷰되며, 통과율만으로는 채택하지 않습니다.
- 테스트 회귀 지표가 악화되는 구간은 우선순위를 자동 상향해 해당 영역을 실험 기능에서 제거 또는 동결합니다.
- 테스트 누락이 반복되면 테스트 설계 템플릿을 리라이팅하고, 회귀가 명확한 항목부터 도메인별 의무 체크로 승격합니다.
- 테스트 설계는 실패 유형 분포(회귀/예외/성능)와 함께 회귀 우선순위를 산출해 리소스를 분배합니다.
- 운영 이슈 빈발 영역은 E2E 자동화 깊이를 올리고, 장시간 실행 시나리오를 분리 운영합니다.
- 회귀 테스트는 임계값이 높은 영역에 집중 투입해 무의미한 케이스 확장을 줄이고 정확도를 높입니다.
- 운영 이슈와 직접 연결되는 테스트만을 핵심 스위트로 유지해 속도와 신뢰 균형을 맞춥니다.
...
- **테스트 경로 강제**: 핵심 비즈니스 플로우는 장애·회귀 시나리오 재현 케이스를 의무 보유합니다.
- **품질 임계 재할당**: 실패 건이 집중되는 영역에 테스트 가중치를 재배분합니다.
...
- [ ] **테스트 경로 가드**: 핵심 플로우 회귀 테스트가 지표 임계치 내에서 운영되는지 확인한다.
- [ ] **리스크 재분배**: 반복 실패 모듈의 테스트 가중치를 증가한다.