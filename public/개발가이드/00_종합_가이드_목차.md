...
> 비유: 이 문서는 **공항의 안내판**과 같습니다. 어디로 가야 하는지(목표), 어느 게이트를 거쳐야 하는지(검증), 누구에게 물어야 하는지(소유권)를 한 화면에 보여줍니다.
...
flowchart TB
  ...
  subgraph L1["Layer 1: System Map"]
    D00["00. 종합 목차"]
  end

  subgraph L2["Layer 2: Core Engineering"]
    D01["01. TypeScript"]
    D02["02. React 19"]
    D03["03. 상태 관리"]
    D04["04. 아키텍처"]
  ...
  subgraph L4["Layer 4: Delivery & Ops"]
    D09["09. 장애대응"]
    D10["10. IaC"]
    D11["11. CI/CD"]
    D12["12. CDN"]
  ...
  subgraph L5["Layer 5: Governance & AI"]
    D15["15. RFC"]
    D16["16. AI 코드리뷰"]
    D17["17. 온보딩"]
    D18["18. AI 워크플로우"]
  ...
  subgraph L6["Layer 6: Product UX"]
    D20["20. 디자인시스템"]
    D21["21. MFE"]
    D23["23. i18n"]
    D24["24. SEO"]
...
React 앱을 새로 도입하거나 버전/빌드 체계를 바꿀 때는
`02. React 19 실무 가이드`의 **13. 리액트 빌드 도입 가이드**를 먼저 적용하고,
`11. CI/CD 파이프라인`의 React 빌드 검증 절차를 연동하세요.

```mermaid
flowchart LR
  A["1단계<br/>품질 원칙"] --> B["2단계<br/>문서 아키텍처"]
  B --> C["3단계<br/>PR 완료 기준"]
  style A fill:#bbdefb
  style B fill:#c8e6c9
  style C fill:#ffe0b2
...
> 00부터 28까지, 프론트엔드 개발의 설계, 구현, 검증, 배포, 운영을 하나의 실무 흐름으로 연결하는 문서 허브입니다.

---

## 추천 항목 (실무 우선순위)

- **현재팀에 먼저 적용**: 현재 팀의 문서 책임 범위와 PR 완료 기준을 기준점으로 잡아, 00-28 각 문서의 충돌 규칙을 1회 정리하세요.
...
```mermaid
flowchart LR
  subgraph Phase1 ["즉시 (1~10일)"]
    A1["팀 문서 책임 범위 정리"] --> A2["PR 완료 기준 게시"]
  end

  subgraph Phase2 ["1~4주차"]
    B1["타입/상태/비동기 규칙 위키"] --> B2["보일러플레이트 구축"]
  end

  subgraph Phase3 ["1~3개월"]
    C1["운영 게이트 샘플 적용"] --> C2["월간 회고 / 누락 제거"]
  end

  A2 --> B1
  B2 --> C1

  style Phase1 fill:#1e293b,stroke:#38bdf8,stroke-width:1px,color:#fff
  style Phase2 fill:#1e293b,stroke:#34d399,stroke-width:1px,color:#fff
  style Phase3 fill:#1e293b,stroke:#fbbf24,stroke-width:1px,color:#fff
```

> 위 gantt는 예시이며, 시작 날짜는 팀 일정에 맞춰 조정합니다.

## 추천 항목 고도화 체크

- `첫 적용` — 가이드 목차와 책임 범위 중 하나를 실제 PR이나 운영 이슈에 붙이고, 변경 전 기준을 먼저 적는다.
- `증거 정리` — 문서 인덱스 diff, 링크 검사 결과, 충돌 규칙 정리를 같은 작업 기록에 남긴다.
- `재점검` — 찾지 못한 문서 링크와 상충 기준 수가 나아졌는지 30일 안에 확인하고 기준을 유지, 수정, 폐기 중 하나로 판정한다.

## 추천 항목 실행 기록 템플릿

- `작업` : 가이드 목차와 책임 범위 적용 범위를 어느 화면, 패키지, 문서에 둘지 적는다.
- `증거` : 문서 인덱스 diff, 링크 검사 결과, 충돌 규칙 정리 중 실제로 남긴 항목만 링크한다.
- `판정` : 유지/수정/폐기 중 하나와 이유를 한 문장으로 남긴다.
- `다음 점검` : 찾지 못한 문서 링크와 상충 기준 수를 다시 볼 날짜와 담당자를 지정한다.
...
| 개별 기술의 구현 세부 기준                  | 각 도메인 문서 01-28                                               |
| 코드 품질 판단 기준                         | Frontend Fundamentals의 가독성, 예측 가능성, 응집도, 결합도 기준   |
| 구조 변경, 워킹그룹/RFC/ADR, 표준 승격/폐기 | [15. 기술 의사결정과 워킹그룹 운영](./15_RFC_의사결정_프로세스.md) |
...
[00 종합 가이드]
 ...
 [08 성능] ──────→ [13 브라우저 호환]
       ...
       └──→ [19 접근성] ──┐
                            │
                            ▼
 [11 CI/CD] → [14 배포] → [09 장애 대응] → [28 Sentry]
       │
       ▼
...
```mermaid
flowchart LR
  subgraph Entry ["1단계: 가이드 시작점"]
    A["요구사항/이슈 제기"] --> B["00 종합 가이드<br/>(문서 품질 / 운영 게이트)"]
    B --> C["01~03 기술 기초<br/>(TypeScript/React/상태관리)"]
  end

  subgraph TrackA ["2단계 (A): 설계 & 구현"]
    D["04 아키텍처 설계"] --> E["05~07 구현/테스트"]
  end

  subgraph TrackB ["2단계 (B): 사용자 경험 (UX)"]
    F["08 성능 최적화"] --> G["13 브라우저 호환성"]
    G --> H["19 웹 접근성"]
  end

  subgraph TrackC ["3단계 (A): 배포 & 운영"]
    I["11 CI/CD 파이프라인"] --> J["14 배포 프로세스"]
    J --> K["09 장애 대응 표준"]
    K --> O["28 Sentry 모니터링"]
  end

  subgraph TrackD ["3단계 (B): 인프라 아키텍처"]
    L["10 인프라/IaC"] --> M["12 CDN 캐시 전략"]
    M --> N["27 다중 개발 서버"]
  end
  ...
  style Entry fill:#0f172a,stroke:#38bdf8,stroke-width:2px,color:#fff
  style TrackA fill:#0f172a,stroke:#34d399,stroke-width:1px,color:#fff
  style TrackB fill:#0f172a,stroke:#f43f5e,stroke-width:1px,color:#fff
  style TrackC fill:#0f172a,stroke:#fbbf24,stroke-width:1px,color:#fff
  style TrackD fill:#0f172a,stroke:#a78bfa,stroke-width:1px,color:#fff
```
...
- **[02. React 19 실무](./02_React19_실무_가이드.md)**: Actions, Server Components, Compiler, Suspense/Error Boundary.
  - **[13. 리액트 빌드 도입 가이드](./02_React19_실무_가이드.md#13-리액트-빌드-도입-가이드)**: 빌드 커맨드, 타입체크, 산출물 증적, 배포 연동.
...
- **[28. Sentry 모니터링 활용](./28_Sentry_모니터링_활용_가이드.md)**: SDK, source map, release health, tracing, replay, alert routing.
- **[29. 표준 라이브러리 스택](./29_표준_라이브러리_스택_가이드.md)**: HTTP·상태·URL·검증·로깅·스타일 표준과 선택 이유.
- **[30. 사례 회고: 저장소 일관성 정렬](./30_사례_전저장소_일관성_정렬_회고.md)**: 여러 저장소 표준 수렴 과정, 공유 config 메커니즘, 검증 게이트 롤아웃 회고.

### Layer 5: Governance & AI

- **[15. 기술 의사결정과 워킹그룹 운영](./15_RFC_의사결정_프로세스.md)**: working group, RFC, ADR, PoC, risk, exit criteria.
...
| Sentry 운영 적용        | 09 -> 28 -> 14 -> 11                         | SDK, source map, release health, trace/replay, alert owner가 연결됨        |
...
```mermaid
flowchart TD
  subgraph Goal1["신규 프로젝트 표준 수립"]
    direction LR
    G1A[00] --> G1B[01] --> G1C[02] --> G1D[04] --> G1E[05] --> G1F[07] --> G1G[11] --> G1H[14]
  end

  subgraph Goal2["기존 프로젝트 품질 개선"]
    direction LR
    G2A[04] --> G2B[07] --> G2C[08] --> G2D[13] --> G2E[19] --> G2F[20]
  end

  subgraph Goal3["배포/운영 안정화"]
    direction LR
    G3A[09] --> G3B[10] --> G3C[11] --> G3D[12] --> G3E[14]
  end

  subgraph Goal6["Sentry 운영 적용"]
    direction LR
    G6A[09] --> G6B[28] --> G6C[14] --> G6D[11]
  end

  subgraph Goal4["AI 협업 도입"]
    direction LR
    G4A[16] --> G4B[18] --> G4C[07] --> G4D[15]
  end

  subgraph Goal5["글로벌 제품 준비"]
    direction LR
    G5A[19] --> G5B[20] --> G5C[23] --> G5D[24] --> G5E[26]
  end

  style Goal1 fill:#e3f2fd
  style Goal2 fill:#e8f5e9
  style Goal3 fill:#fff3e0
  style Goal4 fill:#fce4ec
  style Goal5 fill:#f3e5f5
  style Goal6 fill:#e0f7fa
```
...
flowchart TD
  Q1{"지금 무엇이 가장 급한가?"}
  Q1 -->|"새 프로젝트 시작"| P1["Goal1: 신규 표준 수립"]
  Q1 -->|"기존 프로젝트 개선"| Q2{"무엇이 문제인가?"}
  Q1 -->|"배포가 자주 깨짐"| P3["Goal3: 운영 안정화"]
  Q1 -->|"Sentry 적용/알림 정리"| P7["09 -> 28 -> 14 -> 11"]
  Q1 -->|"AI 활용 정착"| P4["Goal4: AI 협업"]
  Q1 -->|"해외/접근성 확장"| P5["Goal5: 글로벌"]
  Q2 -->|"품질/성능 부족"| P2["Goal2: 품질 개선"]
  Q2 -->|"환경/PR 미리보기 부재"| P6["27 -> 10 -> 11 -> 12 -> 14"]
```
...
flowchart TD
  PR["PR 생성"] --> Type{"변경 유형?"}
  Type -->|타입/계약| C1["tsc + schema test"]
  Type -->|UI| C2["상태 캡처 + interaction"]
  Type -->|API| C3["contract diff + MSW"]
  Type -->|아키텍처| C4["boundary lint + 삭제 범위"]
  Type -->|성능| C5["bundle diff + Web Vitals"]
  Type -->|접근성| C6["axe + keyboard"]
  Type -->|보안| C7["secret scan + audit"]
  Type -->|배포| C8["artifact + rollback 절차"]
...
- `P1(7일 내)` — 가이드 목차와 책임 범위 중 하나를 작은 변경 1건에 적용하고 증거(문서 인덱스 diff)를 남긴다.
- `P2(30일 내)` — 문서 체계 기준을 팀 템플릿, 체크리스트, CI 중 한 곳에 고정한다.
- `P3(90일 내)` — 찾지 못한 문서 링크와 상충 기준 수 추이를 보고 기준을 유지할지 조정할지 결정한다.
- `완료 기준` — 문서 오너가 증거와 철회 조건을 확인했다는 기록을 남긴다.

## 추천 항목 실행 체크리스트

- [ ] `1단계(7일)` : 가이드 목차와 책임 범위 적용 대상을 1개로 좁힌다.
- [ ] `2단계(30일)` : 증거(문서 인덱스 diff, 링크 검사 결과, 충돌 규칙 정리)를 PR, ADR, 회고 중 한 곳에 연결한다.
- [ ] `3단계(60일)` : 찾지 못한 문서 링크와 상충 기준 수가 기준 안에 들어왔는지 확인한다.
- [ ] `문제 대응` : 미달성 사유와 다음 조치, 중단 여부를 같은 기록에 남긴다.

## 추천 항목 실행 운영 규칙

- `실행 게이트` : 새 기준이 어느 문서의 단일 출처인지 먼저 확인한다.
- `승인 체계` : 문서 오너가 영향 범위와 rollback 담당자를 적용 전에 확인한다.
- `재개 조건` : 링크 검사와 책임 범위 표가 모두 맞으면 다음 문서로 확장한다.
- `정지 조건` : 두 문서가 같은 결정을 다르게 말하면 확장을 멈추고 00/15에서 정리한다.
- `리스크 점수` : 상충 문서 수, 깨진 링크 수, 변경 문서 수로 산정한다.
- `리더 승인자` : 문서 체계 오너가 최종 승인 책임을 맡는다.
- `승인 역할` : 문서 체계 작성자, 검토자, 운영 확인자를 분리해 기록한다.
- `재평가 주기` : 월 1회 목차와 검증 스크립트를 함께 점검한다.
