...
| 분류 | 핵심 기술 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [20. 디자인 시스템](./20_디자인_시스템_가이드.md), [08. 성능](./08_성능_최적화_가이드.md) | **AI 도구** | Motion 12 (Framer Motion), GSAP 3.13, Claude Code |
| **핵심 테마** | View Transitions L2 (cross-document), 스크롤 애니메이션, linear() spring, 성능, A/B | **Update** | 2026.05 |

---

> **"좋은 애니메이션은 사용자가 인식하지 못하지만, 없으면 즉시 어색함을 느끼는 보이지 않는 UX다."**
> 본 가이드는 CSS, Web Animations API, Motion 12 (구 Framer Motion), GSAP 3.13, View Transitions API L2 등 2026년 최신 웹 애니메이션 기술을 다룹니다. AI 기반 자동화, A/B 테스트 전략, 성능 최적화, 접근성까지 프로덕션 수준의 모션 구현에 필요한 전 영역을 포함합니다.
>
> **2026년 5월 핵심 변화 요약**
> - **View Transitions Level 2 (Cross-Document)**: Chrome / Edge / Safari 18+ 에서 다중 페이지(MPA) 간 페이지 전환을 JS 없이 구현 가능. Firefox는 2026년 진행 중.
> - **Motion 12 (구 Framer Motion)**: 2026년 3월 v12 정식 릴리즈. View Transitions API와 네이티브 통합, `motion/react`로 패키지명 정리.
> - **CSS `linear()` easing**: 모든 Evergreen 브라우저 Baseline. 스프링 물리(`linear(...)` 다중 스톱)를 JS 없이 표현 가능.
> - **CSS Scroll-Driven Animations**: Chrome/Edge/Safari 정식 지원, Firefox 137부터 부분 지원. GSAP ScrollTrigger 의존도 감소.
> - **GSAP 3.13 + 무료화**: 2024년 Webflow 인수 후 GSAP 전체 플러그인(SplitText, MorphSVG 등)이 상용 라이선스 없이 무료 사용 가능해졌다.
> - **animejs 4.0 / Lottie-React 7**: 양쪽 모두 React 19/Suspense, OffscreenCanvas 지원 강화.
...
6. [GSAP 3.13 타임라인 & 고급 시퀀스](#6-gsap-313-타임라인--고급-시퀀스)
7. [성능 최적화](#7-성능-최적화)
...
| **Motion 12 (구 Framer Motion)** | React 컴포넌트 애니메이션 | ~22 KB (tree-shaking) | 높음 | 훅 제공 | 중간 |
| **GSAP 3.13** | 복잡한 타임라인, SVG, 전 플러그인 무료 | ~25 KB + 플러그인 | 최고 | 수동 | 높음 |
| **View Transitions L1** | SPA 페이지 전환 | 0 KB | 높음 | 브라우저 내장 | 낮음 |
| **View Transitions L2 (MPA)** | 다중 페이지 전환 (cross-document) | 0 KB | 높음 | 브라우저 내장 | 낮음 |
| **Lottie-React 7 / dotLottie** | 일러스트/아이콘 애니메이션 | ~35 KB (canvas 렌더러) | 중간 | 수동 | 낮음 |
| **animejs 4.0** | 경량 타임라인 (GSAP 대안) | ~9 KB | 높음 | 수동 | 낮음 |
...
### Motion 12 (구 Framer Motion) 실전 패턴

> **패키지명 마이그레이션**: 2026년 3월 v12부터 패키지명이 `framer-motion` → `motion`(코어), `motion/react`(React 바인딩)로 정리되었다. 기존 `import { motion } from 'framer-motion'`은 `import { motion } from 'motion/react'`로 교체한다. v11 이하 호환 alias는 유지되지만 v13에서 제거 예정이다.
...
## 6. GSAP 3.13 타임라인 & 고급 시퀀스

GSAP은 복잡한 멀티 스텝 애니메이션 시퀀스, SVG 모핑, 정밀한 타이밍 제어에 적합합니다. Motion 12로 표현하기 어려운 고급 패턴은 GSAP Timeline을 활용합니다.

> **2026년 라이선스 변경**: 2024년 Webflow의 GSAP 인수 이후, 2026년 5월 기준 **모든 GSAP 플러그인(SplitText, MorphSVG, DrawSVG, Physics2D 등)이 무료**로 사용 가능하다. 기존에 유료였던 Club GreenSock 멤버십이 폐지되었고, 상용 프로젝트에서도 별도 라이선스 비용이 발생하지 않는다. `npm i gsap`만으로 모든 플러그인을 즉시 사용할 수 있다.
>
> **선택 가이드 2026**
> - Motion 12 / CSS Scroll-Driven으로 충분한 케이스에서는 GSAP 도입을 자제 (번들 사이즈 + 메인 스레드 비용).
> - 다음 케이스에서는 GSAP이 여전히 우위: SVG path morph, FLIP, DrawSVG, 복잡한 SplitText, Lenis 기반 부드러운 스크롤, ScrollSmoother.
...
애니메이션의 일관성을 보장하기 위해 duration, easing, delay 등을 디자인 토큰으로 관리합니다. 하드코딩된 값 대신 토큰을 사용하면 전역 조정이 가능하고 A/B 테스트도 용이합니다.
...
| # | 항목 | 확인 |
|---|------|------|
| 1 | `framer-motion` → `motion/react` (Motion 12) 마이그레이션 완료 | [ ] |
| 2 | MPA 전환에 `@view-transition { navigation: auto }` 적용 (Level 2) | [ ] |
| 3 | 스프링 곡선을 `linear()` easing으로 표현 (JS 라이브러리 제거 가능 여부 확인) | [ ] |
| 4 | CSS Scroll-Driven Animations로 ScrollTrigger 대체 가능한 부분 식별 | [ ] |
| 5 | GSAP 사용 시 모든 플러그인 무료 라이선스 적용 확인 (Webflow 인수 후 2026 정책) | [ ] |
