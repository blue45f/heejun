# 25. 웹 애니메이션 & 모션 가이드 (2025-2026 Edition)

| 분류 | 핵심 기술 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [20. 디자인 시스템](./20_디자인_시스템_가이드.md), [08. 성능](./08_성능_최적화_가이드.md) | **AI 도구** | Framer Motion, GSAP, Claude Code |
| **핵심 테마** | View Transitions, 스크롤 애니메이션, 성능 최적화, A/B 테스트 | **Update** | 2026.04 |

---

> **"좋은 애니메이션은 사용자가 인식하지 못하지만, 없으면 즉시 어색함을 느끼는 보이지 않는 UX다."**
> 본 가이드는 CSS, Web Animations API, Framer Motion, GSAP, View Transitions API 등 2025-2026 최신 웹 애니메이션 기술을 다룹니다. AI 기반 자동화, A/B 테스트 전략, 성능 최적화, 접근성까지 프로덕션 수준의 모션 구현에 필요한 전 영역을 포함합니다.
...
| **CSS Scroll-Driven** | 스크롤 연동 (2025 네이티브) | 0 KB | 최고 | 기본 지원 | 중간 |
...
> **참고**: View Transitions API는 2025년 기준 Chrome, Edge, Safari 18+에서 지원됩니다. [13. 브라우저 호환성 가이드](./13_브라우저_호환성_가이드.md)에서 Baseline 지원 현황을 확인하세요.
...
> **참고**: CSS Scroll-Driven Animations는 Chrome 115+에서 지원됩니다. 미지원 브라우저를 위해 IntersectionObserver 기반 JS 폴백을 함께 구현하세요. 성능 측정은 [08. 성능 최적화 가이드](./08_성능_최적화_가이드.md)를 참고합니다.
...
GSAP은 복잡한 멀티 스텝 애니메이션 시퀀스, SVG 모핑, 정밀한 타이밍 제어에 적합합니다. Framer Motion으로 표현하기 어려운 고급 패턴은 GSAP Timeline을 활용합니다.
...
| 3 | View Transitions 미지원 브라우저 ([13. 호환성 가이드](./13_브라우저_호환성_가이드.md)) 폴백 동작 확인 | [ ] |
