...
이 문서를 처음 읽는다면 먼저 아래 3가지만 기억하세요.

1. **타입 검사는 버그를 줄여주지만, 런타임에서는 꼭 별도의 검증이 필요합니다.**
   - API 응답이 엉뚱해도 컴파일은 통과할 수 있습니다.
2. **`strict`는 기본값입니다.**
   - 팀에서 같은 규칙을 쓰면 코드리뷰 갈등이 줄고, 같은 버그를 반복하지 않습니다.
3. **`as`, `!`, `any`는 임시 방편입니다.**
   - 꼭 필요한 곳은 분리하고 이유를 문서에 남겨야 합니다.

이 3가지를 먼저 정하면 이후 1~9장 문항을 읽을 때 기준이 생깁니다.
