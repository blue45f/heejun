# 20. 디자인 시스템 가이드 (2025-2026 Edition)

| 분류 | 심화 주제 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [02. React 19](./02_React19_실무_가이드.md), [19. 웹 접근성](./19_웹_접근성_가이드.md), [25. 애니메이션](./25_웹_애니메이션_모션_가이드.md) | **AI 도구** | Tailwind v4, Storybook |
| **핵심 테마** | Tailwind v4, Design Tokens, Headless UI, Component-Driven, Dark Mode | **Update** | 2025.04 |

---

> **"디자인 시스템은 컴포넌트 모음이 아니라, 팀의 일관된 언어이자 효율적인 협업 도구다."**
> 본 가이드는 Tailwind CSS v4와 최신 Headless UI 라이브러리를 활용하여 견고하고 확장 가능한 디자인 시스템을 구축하는 방법을 다룹니다. 다크 모드, 반응형 토큰, 컴포넌트 아키텍처, 배포 전략까지 실무에 필요한 전 영역을 포함합니다.
...
디자이너와 개발자 간 토큰 불일치를 방지하기 위해 **단일 소스(Single Source of Truth)**를 유지합니다.

```
Figma Variables  ──►  tokens.json  ──►  CSS Custom Properties
(디자이너 작업)      (중간 포맷)        (개발자 코드)
      ▲                   │
      └───── 역방향 동기화 ──┘
```

**권장 도구:**
- **Tokens Studio for Figma** (구 Figma Tokens): Figma 플러그인으로 토큰을 JSON 관리
- **Style Dictionary**: Amazon 오픈소스, 토큰 JSON을 여러 플랫폼 코드로 변환

```json
// tokens/color.json - Style Dictionary 입력 형식
{
  "color": {
    "primitive": {
      "blue": {
...
## 4. 문서화 및 검증: Storybook

컴포넌트의 독립적인 개발과 테스트를 위해 Storybook 8을 사용합니다.
