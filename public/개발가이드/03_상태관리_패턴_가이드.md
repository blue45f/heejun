...
> **일상 비유**: 학생증 발급 절차와 유사합니다. 먼저 신분증을 확인(현재 사용자)해야 학번을 발급(주문 목록 조회)할 수 있고, 학번이 있어야 강의 등록(주문 상세 페치)이 가능합니다. 앞 단계가 끝나기 전에는 뒤 단계를 시작할 수 없습니다.
...
표준 Signals 제안은 **현재 공식 문서 기준 TC39 Stage 1**에 머물러 있습니다. 여러 프레임워크에서 자체 구현체가 활발하지만, **React 본체는 공식적으로 Signal을 채택하지 않았습니다.** 따라서 React 코드베이스에서는 표준화 전까지 Signal API를 전면 도입하지 않고 adapter 경계 안에서 실험합니다.
