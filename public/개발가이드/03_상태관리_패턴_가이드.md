...
서버에서 `prefetch`를 시작하되, **데이터가 준비되기 전에 일단 HTML을 흘려보내고** Suspense 경계가 해소되는 대로 클라이언트 캐시로 스트리밍합니다. Next.js 15+, TanStack Start, Remix 등에서 활용 가능합니다.
...
| :--- | :--- | :--- | :--- |
| **TanStack Query v5** | 서버 상태 캐시 | API 응답 / RSC 스트리밍 | 서버 데이터는 무조건 여기 |
...
표준 Signals 제안은 **2026-05 기준 TC39 Stage 1**에 머물러 있습니다. Solid, Preact, Vue, Angular 등에서 자체 구현체가 활발하지만, **React 본체는 공식적으로 Signal을 채택하지 않았습니다.** Meta 측은 React Compiler 1.0이 자동 메모이제이션을 충분히 처리한다고 보고 있어, 적어도 2026년 안에 React가 Signal API를 노출할 가능성은 낮습니다.
