# 21. 마이크로 프론트엔드 가이드 (2026 Edition)

| 분류 | 핵심 기술 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [22. 모노레포](./22_모노레포_운영_가이드.md), [04. 아키텍처](./04_아키텍처_설계_패턴.md), [11. CI/CD](./11_CICD_파이프라인_표준.md) | **AI 도구** | Module Federation 2.x, rsbuild, Claude Code |
| **핵심 테마** | Module Federation 2.x (Stable), Runtime API, Native Federation, 독립 카나리 배포, RSC 통합 | **Update** | 2026.05 |

---

> **"마이크로 프론트엔드는 독립적인 팀이 독립적으로 배포할 수 있는 프론트엔드 아키텍처이다. 핵심은 기술이 아니라 조직의 자율성과 배포 독립성이다."**
> 본 가이드는 **Module Federation 2.x**(2026년 4월 Stable 릴리스)를 중심으로, 모놀리스에서 마이크로 프론트엔드로의 안전한 마이그레이션 전략과 독립 카나리 배포, 성능 최적화, 공유 패턴을 실무 코드와 함께 다룹니다.
>
> **2026년 MFE 생태계 변화 요약**
> - **Module Federation 2.x Stable** (2026.04): 런타임이 빌드 도구로부터 완전히 분리되어 Webpack/Rspack/Rolldown/Rsbuild/Vite/Metro 전 플랫폼에서 동일하게 동작
> - **Runtime API**: `loadRemote`, `preloadRemote`, `registerRemotes`, `prefetch`, cache API 통합. 빌드 플러그인 없이 순수 런타임 등록 가능
> - **`@module-federation/rsbuild-plugin`**: rsbuild에서 1급 지원, mf-manifest 기반 타입 자동 생성
> - **Native Federation (ESM)**: Import Maps 기반 대안, Webpack 5.97+에서 ESM 호환성 개선
> - **vite-plugin-federation 1.0**: Vite 환경에서 프로덕션급 MFE 지원 (2025년 1.0 출시)
> - **RSC + MFE 통합 패턴**: Server Component에서 데이터 프리페치, Client 경계에서 리모트 로드 - Next.js 15 / Modern.js 정식 지원
...
10. [마이그레이션 전략: 모놀리스에서 MFE로](#10-마이그레이션-전략-모놀리스에서-mfe로)
11. [테스팅 전략](#11-테스팅-전략)
12. [체크리스트](#12-체크리스트)
...
2026년 4월 Module Federation 2.x Stable 릴리스로 런타임이 빌드 도구에서 완전히 분리되었고, Webpack/Rspack/Rolldown/Rsbuild/Vite/Metro 전 번들러에서 일관된 동작을 보장합니다.

| 비교 항목 | Module Federation 2.x (2026 Stable) | Native Federation (ESM) |
|---|---|---|
...
| **SSR 지원** | Node Federation, Next.js 15 / Modern.js 통합 | 제한적 |
| **프로덕션 사례** | 다수 (Bytedance, Microsoft, AWS 등 대규모 채택) | 증가 중 (Angular 19+ 기본 채택) |
...
- **Module Federation 2.x**: 대규모 조직, 복잡한 공유 의존성 관리, RSC 통합 또는 다중 번들러 환경(Vite + Rspack 혼재 등)에서 일관된 런타임이 필요할 때. 2.x Stable 이후 사실상 업계 표준.
- **Native Federation (Import Maps)**: 번들러 독립성과 ESM 네이티브 성능을 최우선으로 할 때. 특히 Angular 19+ 환경(공식 채택) 또는 Deno/Bun 기반 모던 스택.
- **vite-plugin-federation 1.0**: Vite 단독 환경에서 가벼운 MFE를 시작할 때(개발 속도 우선). 단, MF 2.x Vite 통합이 안정화되면서 사용처가 줄어드는 추세.
...
2026년 5월 기준 `@module-federation/rsbuild-plugin`, `@module-federation/enhanced`(Webpack), `@module-federation/vite`가 정식 출시되어 모든 주요 번들러에서 동일한 API를 사용할 수 있습니다. 아래는 rsbuild 기준 설정이며, Webpack 사용자는 `@module-federation/enhanced/webpack`을 import하면 동일하게 동작합니다.
...
export class RemoteErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo): void {
    ...
    // 에러 리포팅 (참고: [09. Sentry](./09_장애_대응_및_Sentry_표준.md))
...
| 주제 | 링크 |
|---|---|
| Module Federation 2.x (Stable 2026.04) | https://module-federation.io |
...
| vite-plugin-federation 1.0 | https://github.com/originjs/vite-plugin-federation |
| Native Federation (Angular 19+ 기본) | https://www.npmjs.com/package/@softarc/native-federation |
| Single-SPA 6.x | https://single-spa.js.org |
| React 19 Server Components | https://react.dev/reference/rsc/server-components |
| Next.js 15 + MFE 통합 | https://module-federation.io/practice/frameworks/next/nextjs-mf |
...
| [09](./09_장애_대응_및_Sentry_표준.md) | 장애 대응 및 Sentry 표준 | 에러 모니터링, 장애 대응 |
| [11](./11_CICD_파이프라인_표준.md) | CI/CD 파이프라인 표준 | 빌드/배포 파이프라인 |
| [12](./12_CloudFront_캐시_전략.md) | CloudFront 캐시 전략 | CDN 배포, 캐시 무효화 |
