# 21. 마이크로 프론트엔드 가이드 (2025-2026 Edition)

| 분류 | 핵심 기술 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [22. 모노레포](./22_모노레포_운영_가이드.md), [04. 아키텍처](./04_아키텍처_설계_패턴.md), [11. CI/CD](./11_CICD_파이프라인_표준.md) | **AI 도구** | Module Federation, Claude Code |
| **핵심 테마** | Module Federation 2.1, 독립 카나리 배포, 마이그레이션 전략 | **Update** | 2026.04 |

---

> **"마이크로 프론트엔드는 독립적인 팀이 독립적으로 배포할 수 있는 프론트엔드 아키텍처이다. 핵심은 기술이 아니라 조직의 자율성과 배포 독립성이다."**
> 본 가이드는 Module Federation 2.1을 중심으로, 모놀리스에서 마이크로 프론트엔드로의 안전한 마이그레이션 전략과 독립 카나리 배포, 성능 최적화, 공유 패턴을 실무 코드와 함께 다룹니다.
...
- **Module Federation 2.1**: 대규모 조직, 복잡한 공유 의존성 관리가 필요할 때. Rspack/Webpack 생태계를 이미 사용 중이라면 자연스러운 선택.
- **Native Federation**: 번들러 독립성이 중요하고, 앱 간 공유 의존성이 단순할 때. ESM 네이티브 성능을 최우선으로 고려할 때.
