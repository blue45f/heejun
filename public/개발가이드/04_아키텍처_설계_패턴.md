...
이 문서의 기본값은 엄격한 6계층 분리가 아니라 **기능 중심 co-location**입니다. 컴포넌트, hook, API, 타입, 상수, MSW handler, 테스트가 같은 이유로 바뀐다면 같은 feature/domain 폴더 아래에 둡니다. 라우트는 얇은 조합 계층으로 유지하고, 실제 변경 책임은 domain feature가 가집니다.
...
| `domains/{domain}/{feature}` | 해당 기능의 UI, hook, query, mutation, model, MSW handler, 테스트 | 이 기능을 삭제할 때 폴더 하나를 지우면 충분한가? |
...
| 응집도 | [함께 수정되는 파일을 같은 디렉토리에 두기](https://frontend-fundamentals.com/code-quality/code/examples/code-directory.html) | feature/domain 폴더 안에 UI, hook, API, mock, test를 같이 둡니다. | 삭제나 롤백이 여러 종류별 폴더를 수색해야 가능한가? |
...
이 문서의 기본 구조는 0.3의 기능 중심 co-location입니다. FSD는 프로젝트를 6개의 계층(Layers)으로 나누어 의존성을 단방향으로 흐르게 하는 선택지이며, 이미 FSD를 채택한 저장소나 충분히 큰 제품 영역에서만 전체 계층을 적용합니다. 새 기능을 만들 때는 먼저 변경 단위와 삭제 단위를 확인하고, 필요할 때만 FSD의 계층 이름을 사용합니다.

> **FSD 2.1에서 참고할 점 (Pages First)**: 다른 페이지에서 재사용되지 않는 UI 블록·폼·데이터 로직은 별도 widgets/features로 끌어올리지 않고 해당 페이지 또는 domain feature 안에 그대로 둡니다. `@x` 크로스 임포트와 `Steiger`는 FSD 저장소에서만 사용하고, 일반 co-location 구조에서는 `eslint-plugin-boundaries`, package export, dependency graph로 같은 목적을 달성합니다.
...
| **domains/{domain}/{feature}/api** | Server | DB/API secret, cache tag, server action을 클라이언트 번들에서 격리 |
...
```typescript
// domains/user/profile/api/apiUserRepository.ts (실제 구현체)
...
api/auth.ts (login 부분)   → domains/auth/login/api/loginAction.ts
