# 04. 아키텍처 설계 패턴 (2026 Edition)

| 분류 | 아키텍처 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [02. React 19](./02_React19_실무_가이드.md), [07. 테스팅](./07_테스팅_가이드.md), [03. 상태관리](./03_상태관리_패턴_가이드.md) | **도구 원칙** | 벤더 중립 |
| **핵심 테마** | FSD 2.1 (Pages-first), Clean Architecture, RSC 계층 설계, PPR | **Update** | 2026.05 |
...
### 2.6 RSC + TanStack Query 하이브리드 (2026 표준 패턴)
...
### 4.3 Steiger — FSD 공식 아키텍처 린터 (2026 권장)
...
- **의존성 경계 자동 점검**: public API와 internal module 경계를 모듈 그래프 규칙으로 검사하고, 순환 의존성은 PR 단계에서 fail하도록 템플릿화합니다.
- **RSC/SSR 호환성 레이어**: 렌더링 경로별 직렬화·캐시 계약을 도메인별로 문서화해 PWA/SEO/성능 기준과 연결합니다.
- **아키텍처 실험 비용**: 이벤트 소유권 분리, 마이크로 아키텍처 분할은 도입 전 복잡도 증가 지표와 교체 비용을 함께 계산합니다.
- **장애 격리 회로**: 상위 레이어 에러 경계, fallback UI, circuit-breaker 경로를 모듈 단위에서 정의하고 실패 예산을 기록합니다.
...
- 모듈 경계 비용 산정: 기능 추가 시 의존도 증가를 빌드/테스트 비용과 연동해 승인 규칙을 적용합니다.
- 계층형 회복 경로: API/도메인/UI 레이어별 폴백 전략과 실패 전파 중단점을 문서화합니다.
- 계약 기반 아키텍처: 패키지 public API는 버전·변경 로그를 남기고 breaking change는 사전 고지합니다.
- PPR·SSR 상호운영성: 렌더링 경로별 캐시 무효화와 SEO/성능 KPI 충돌을 테스트 템플릿으로 점검합니다.
- 실험 통제 규칙: 한 번에 여러 아키텍처 실험을 병렬로 띄우지 않고 rollback·abort 조건을 고정합니다.

- 경계 기반 설계를 도입할 때 도메인 이벤트 계약과 외부 통신 계약을 동일 레벨에서 버저닝해, 변경 충돌 시 계약 드리프트를 즉시 탐지합니다.
- 단방향 의존성 가이드라인과 점진적 분해 전략을 유지해 기능 확장 시 영향 면을 좁히고, 모듈 간 동기화 비용을 계산 지표로 추적합니다.
- 플랫폼 독립 계층을 운영하면 인프라 교체 시 비즈니스 로직과 정책 변경을 분리해 장애 전파를 줄입니다.
- 도메인 경계마다 의존성 허용 목록을 두고, 경계 위반 시 자동 알림을 통해 설계 문서와 코드가 같은 주기에 갱신되게 만듭니다.
- 기능 단위 분해 후에도 실패 모드 분석이 쉬운지 주기적으로 점검해 리팩터링 리스크를 수치화합니다.
- 의존성 경계 변경은 사전 영향도 분석표와 함께 진행하고, 영향이 큰 변경은 단계형 마이그레이션 플랜으로 분할합니다.
- 설계 변경 후 1주일 내 관측 지표 악화 여부를 점검해 설계 문서와 구현의 정합성을 검증합니다.
- 경계 위반 사례는 아키텍처 리뷰에서 즉시 차단하고 수정 전까지 배포 의사결정을 보류합니다.
- 기능 추가 시 복잡도 증가와 테스트 비용을 함께 평가해 설계 승인 시점을 조정합니다.
...
- **경계 재설계 룰**: 설계 변경은 의존성 방향성 위반이 없을 때만 확장하고, 충돌 건은 표준 거부 항목으로 처리합니다.
- **기능 분해 승인**: 경계 분해는 테스트 전략이 선행될 경우에만 본선 적용됩니다.
...
- [ ] **경계 규칙 감시**: 의존성 경계 문서와 실제 구현이 일치하는지 분기별로 감사한다.
- [ ] **점진 변경 확정**: 복잡도 증가 구간에서 배포 범위와 회귀 실험을 분리한다.