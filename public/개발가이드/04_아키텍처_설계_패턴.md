...
- `첫 적용` — feature boundary와 shared 승격 조건 중 하나를 실제 PR이나 운영 이슈에 붙이고, 변경 전 기준을 먼저 적는다.
- `증거 정리` — import boundary 결과, 폴더 이동 diff, owner 승인를 같은 작업 기록에 남긴다.
- `재점검` — deep import 수, shared 변경 동반 PR 수, dead code 제거량가 나아졌는지 30일 안에 확인하고 기준을 유지, 수정, 폐기 중 하나로 판정한다.

## 추천 항목 실행 기록 템플릿

- `작업` : feature boundary와 shared 승격 조건 적용 범위를 어느 화면, 패키지, 문서에 둘지 적는다.
- `증거` : import boundary 결과, 폴더 이동 diff, owner 승인 중 실제로 남긴 항목만 링크한다.
- `판정` : 유지/수정/폐기 중 하나와 이유를 한 문장으로 남긴다.
- `다음 점검` : deep import 수, shared 변경 동반 PR 수, dead code 제거량를 다시 볼 날짜와 담당자를 지정한다.
...
| 되돌리기 어려운 구조 변경의 승인/폐기 기준                   | [15. 기술 의사결정과 워킹그룹 운영](./15_RFC_의사결정_프로세스.md)                                          |
...
Frontend Fundamentals의 17개 예시는 개별 코드 스타일뿐 아니라 폴더 구조와 리뷰 경계의 판단 기준으로도 사용합니다. PR 리뷰에서는 아래 질문에 답할 수 있어야 합니다.
...
> **일상 비유**: 서랍을 "양말 서랍 / 셔츠 서랍 / 바지 서랍"으로 나누면 옷이 늘 때 곤란합니다. 대신 "출근복 한 벌 / 운동복 한 벌"처럼 같이 입는 옷을 한 칸에 묶으면 옷을 갈아입을 때 한 칸만 열면 됩니다. feature co-location은 같이 바뀌는 코드를 한 칸에 묶는 일입니다.

#### 폴더 구조 종속성 다이어그램

```mermaid
flowchart TD
  subgraph Routes[app/routes — Shell]
    R1[transfer/page.tsx]
    R2[account/page.tsx]
  end
  subgraph Domains[domains/]
    subgraph Transfer[transfer]
      TS[send/index.ts<br/>public API]
      TH[history/index.ts]
      TSH[_shared/]
    end
  ...
  R1 -->|public API only| TS
  R2 -->|public API only| AL
  TS --> TSH
  TS --> UI
  TS --> LIB
  TS --> HTTP
  ...
  AL --> HTTP
  TS -. 금지 .-> AD
  classDef forbid stroke:#dc2626,stroke-width:2px,stroke-dasharray: 4 2
  class TS,AD forbid
```
...
> **일상 비유**: 부서간 결재선과 같습니다. 영업팀이 인사팀 책상을 뒤져서 직원 정보를 가져오면 위반이지만, 인사팀이 공식으로 발급한 사번 리스트(public API)를 받는 건 정상입니다. 외부 결재선(라우트 조합)이 두 부서를 묶을 수도 있습니다.

#### 도메인 의존성 방향 그래프

이 그림은 허용되는 의존 방향과 금지되는 cross-import를 한눈에 보여줍니다.

```mermaid
flowchart TD
  subgraph Routes [app/routes - orchestration]
    R1[transfer page]
    R2[account page]
  end
  subgraph Domains [domains - business]
    subgraph Transfer [transfer]
      TS[send public API]
      TH[history public API]
    end
    subgraph Account [account]
      AL[list public API]
      AD[detail public API]
    end
  end
  ...
  R1 -->|allowed| TS
  R1 -->|allowed cross-domain via shell| AL
  R2 -->|allowed| AL
  R2 -->|allowed cross-domain via shell| TS
  TS -->|allowed| UI
  TS -->|allowed| Lib
  TS -->|allowed| Http
  TH -->|allowed| UI
  AL -->|allowed| UI
  AL -->|allowed| Http
  Infra -->|allowed| Shared

  TS -. forbidden cross-domain .-> AL
  AL -. forbidden cross-domain .-> TS
  Lib -. forbidden upward .-> TS
  UI -. forbidden upward .-> AL
```
...
React 19 시대의 아키텍처에서는 서버/클라이언트 경계를 의식적으로 설계합니다. route, domain feature, infrastructure 경계와 RSC 배치를 결합하면 데이터 소유권과 번들 경계가 자연스러워집니다.

> **일상 비유**: 식당에서 주방(서버)은 재료 손질·조리(데이터·시크릿)를 담당하고, 홀(클라이언트)은 손님 응대·주문 변경(상호작용)을 담당합니다. 주방 도구를 홀로 가지고 나가지 않듯이, 비밀키·DB 커넥션을 클라이언트로 보내지 않습니다.

> 상세 내용은 [02. React 19 실무 가이드](./02_React19_실무_가이드.md)를 참고하세요.

#### Server/Client 경계 분리 흐름

```mermaid
flowchart LR
  subgraph Server[Server 영역]
    R[app/routes shell]
    RE[domains/.../remotes<br/>'server-only']
    SC[Server Components<br/>async/await fetch]
    IN[infrastructure/http]
  end
  subgraph Boundary["직렬화 경계 (props)"]
    P[(직렬화 가능 데이터<br/>string/number/array/object)]
  end
  subgraph Client[Client 영역]
    CC[Client Components<br/>'use client']
    HK[hooks 상태/이벤트]
    QC[TanStack Query 갱신]
  end
  R --> SC
  SC --> RE
  RE --> IN
  ...
  HK --> QC
  QC -. mutation .-> RE
  classDef forbidden stroke:#dc2626,stroke-dasharray:4 2
  X[함수 / 클래스 인스턴스<br/>Map/Set/Symbol] -. 금지 .-> P
  class X forbidden
```
...
Next.js App Router에서 자주 쓰이는 운영 패턴은 **"서버에서 핵심 데이터 + 클라이언트에서 인터랙티브 갱신"** 하이브리드입니다. 서버 컴포넌트가 첫 페이로드를 prefetch한 뒤 `HydrationBoundary`로 캐시를 전달하면, 클라이언트는 동일한 쿼리 키로 즉시 hydrate되고 이후 mutation/filter/실시간 갱신만 맡습니다. Next.js 15+ App Router 프로젝트에도 같은 패턴을 점진 적용할 수 있습니다.
...
API 통신이나 외부 라이브러리 의존성은 직접 구현체에 의존하지 않고 **인터페이스로 분리**합니다. 이렇게 두면 테스트가 쉬워지고 구현체 교체도 단순해집니다.

> **일상 비유**: 콘센트(인터페이스)는 가전제품(구현체)이 무엇이든 동일한 모양으로 받아들입니다. 가전이 바뀌어도 벽은 그대로입니다. DIP는 코드의 벽을 "특정 가전"이 아니라 "콘센트 규격"에 맞춰 두는 일입니다.

#### DIP 어댑터 패턴 구조

```mermaid
flowchart TD
  subgraph Domain[도메인 / 사용처]
    H[useEditUser hook]
    IFace[UserRepository<br/>인터페이스]
  end
  subgraph Adapters[어댑터 구현체]
    A1[ApiUserRepository<br/>실 서버 호출]
    A2[MockUserRepository<br/>테스트 인메모리]
    A3[StorageUserRepository<br/>오프라인 캐시]
  end
  H --> IFace
  IFace -. impl .-> A1
  IFace -. impl .-> A2
  IFace -. impl .-> A3
  R[런타임: factory가 환경별 어댑터 주입] --> H
  T[테스트] --> A2
  P[프로덕션] --> A1
```
...
> **일상 비유**: 공항의 보딩 게이트는 항공권을 자동으로 확인합니다. 직원의 눈으로만 검사하면 한두 명이 잘못된 게이트를 통과합니다. 경계 검증은 매 PR에 게이트를 자동으로 세우는 일입니다.
...
```mermaid
flowchart LR
  A[개발자 push] --> B[로컬 pre-commit]
  B --> C{Biome check<br/>형식/일반 린트}
  C -- 실패 --> Z[수정 요청]
  C -- 통과 --> D[PR 생성]
  D --> E[CI 게이트]
  E --> F[eslint-plugin-boundaries<br/>계층 의존 검사]
  E --> G[FSD 저장소만:<br/>Steiger 검사]
  E --> H[Biome / TypeScript<br/>noUndeclaredDependencies]
  F -- 위반 --> X[보고서: 위반 위치, 룰명]
  G -- 위반 --> X
  H -- 위반 --> X
  X --> Z
...
```mermaid
flowchart TD
  subgraph Feature["domains/transfer/send/"]
    A[components/TransferAmount.tsx]
    B[hooks/useAmount.ts]
    C[remotes/createTransfer.ts]
    D[model/schema.ts]
    E[index.ts]
  end
  E -. re-export .-> A
  E -. re-export .-> D
  subgraph External[외부 소비자]
    R[app/routes/transfer/page.tsx]
    O[domains/transfer/history/...]
  end
  R -->|허용| E
  O -->|허용| E
  R -. 금지: deep import .-> A
  R -. 금지: deep import .-> B
  O -. 금지: 다른 feature 내부 .-> B
...
domain feature 간 참조가 필요하면 내부 파일을 직접 import하지 말고 각 feature의 `index.ts` public API로 상위 orchestration에서 조합합니다. 같은 domain 아래 feature끼리도 기본은 직접 의존 금지입니다.
...
> **참고**: Biome는 `eslint-plugin-boundaries`와 같은 계층 간 import 제한 기능을 아직 내장하지 않습니다. 계층 검증은 **Biome(포맷팅/린트) + eslint-plugin-boundaries(계층 검증)** 조합으로 맡기는 편이 낫습니다.
...
#### 4.3.1 UI 컨벤션도 린트로 강제한다 — 네이티브 다이얼로그 금지

아키텍처 경계뿐 아니라 **UI 컨벤션**도 문서가 아니라 린트로 잠근다. 대표 예가 `window.confirm/alert/prompt`다. 네이티브 다이얼로그는 테마/브랜딩 불가, 메인 스레드 블로킹, OS별 비일관, 접근성 한계가 있어 제품에선 **브랜드 확인 다이얼로그**로 대체한다. 한번 마이그레이션한 뒤에는 ESLint `no-restricted-globals`로 재유입을 차단한다:

```js
// eslint.config.js (웹 코드 블록)
'no-restricted-globals': [
  'error',
  { name: 'confirm', message: 'useConfirm()/ConfirmDialog를 쓰세요.' },
  { name: 'alert', message: 'Toast/Dialog를 쓰세요.' },
  { name: 'prompt', message: '입력 다이얼로그/폼을 쓰세요.' },
],
```

`no-restricted-globals`는 스코프를 보므로 `const confirm = useConfirm()` 같은 **로컬 섀도잉은 오탐하지 않는다** — 전역 호출만 막는다. 권장 대체 패턴은 앱 루트에 한 인스턴스를 두는 **명령형 `useConfirm()` 훅**(`Promise<boolean>` 반환, 기존 Dialog/Sheet 프리미티브 위에 구축)으로, 호출부는 `if (!(await confirm({ title, danger: true }))) return;` 형태가 된다. 미마이그레이션 항목이 남으면 해당 전역만 우선 제외하고 점진 차단한다.
...
> **일상 비유**: Type-based는 모든 옷을 종류별 서랍(셔츠/바지/양말)에 따로 두는 방식, co-location은 출근복 한 벌을 같은 칸에 묶어두는 방식, FSD는 호텔처럼 층별로 역할(체크인 카운터/객실/룸서비스)을 강제하는 방식입니다.

이 그림은 세 구조에서 같은 기능의 파일들이 어떻게 흩어지거나 모이는지를 비교합니다.

```mermaid
flowchart LR
  subgraph Type [Type-based 구조]
    direction TB
    TC[components/LoginForm]
    TH[hooks/useLogin]
    TA[api/auth.ts]
    TT[tests/login.test]
    TC -. 같은 기능이지만 분리 .- TH
  ...
  subgraph Coloc [기능 중심 co-location]
    direction TB
    CR[domains/auth/login/<br/>components/LoginForm<br/>hooks/useLogin<br/>remotes/loginAction<br/>mocks/handlers<br/>tests/login.test<br/>index.ts]
  end
  subgraph FSD [FSD 구조]
    direction TB
    FP[pages/login]
    FF[features/login-form]
    FE[entities/user]
    FS[shared/ui]
    FP --> FF
    FF --> FE
...
> **일상 비유**: 이사할 때 모든 짐을 한 트럭에 실으려 하면 사고가 납니다. 책장 먼저, 옷 다음, 마지막에 깨지기 쉬운 것 — 단계적으로 옮기듯 마이그레이션도 단계를 나눠야 합니다.

#### 4단계 마이그레이션 흐름

이 그림은 type-based 구조에서 기능 중심 co-location으로 옮기는 작업의 권장 순서와 게이트를 보여줍니다.

```mermaid
flowchart TD
  S0[현재: type-based src/<br/>components, hooks, utils, api] --> S1[Step 1<br/>도메인 무관 공통을 shared/로 이동]
  S1 --> G1{게이트<br/>기존 import 자동 변환}
  G1 -- 통과 --> S2[Step 2<br/>최근 PR 변경 단위로<br/>domains/x/y feature 생성]
  G1 -- 실패 --> S1
  S2 --> G2{게이트<br/>boundary lint 도입}
  G2 -- 통과 --> S3[Step 3<br/>MSW handler/fixture/test<br/>feature로 co-location]
  G2 -- 실패 --> S2
  S3 --> G3{게이트<br/>contract test 통과}
  G3 -- 통과 --> S4[Step 4<br/>public API index.ts 노출<br/>deep import 차단]
  G3 -- 실패 --> S3
  S4 --> Done[완료: feature 단위 변경 가능]
  S0 -. re-export 브리지로 .-> S4
```
...
> **팁**: 마이그레이션 중에는 기존 경로에서 `export { ... } from '새경로'`로 re-export하여 점진 전환하면 대규모 diff를 피할 수 있습니다.
...
shared 계층의 UI 컴포넌트는 **유연하고 재사용 가능**하게 설계해야 합니다. 컴포넌트가 도메인 단어나 비즈니스 분기를 품으면 공유의 의미가 사라지고, 너무 빈 깡통이면 사용처마다 보일러플레이트가 늘어납니다.

> **일상 비유**: 사무용 책상은 도면(부품 분리, 조립 옵션)이 표준이라 회의실·개인 자리·로비 어디에 놓아도 다른 가구와 어우러집니다. Compound·Headless 패턴은 그 표준 책상을 코드로 만드는 방식입니다.
...
> **일상 비유**: 가구 매장의 모듈 가구처럼, Card.Root, Card.Header, Card.Body, Card.Footer는 각각 독립된 부품이지만 같은 디자인 언어로 결합됩니다. 사용자는 필요한 부품만 골라 조합할 수 있습니다.

#### Compound 컴포넌트의 의존성 구조

이 그림은 Card 루트와 서브 컴포넌트들이 Context를 통해 어떻게 정보를 공유하는지 보여줍니다.

```mermaid
flowchart TD
  Root[Card.Root<br/>variant 결정] --> Ctx[CardContext.Provider<br/>variant 값 보유]
  Ctx --> Hdr[Card.Header]
  Ctx --> Body[Card.Body]
  Ctx --> Ftr[Card.Footer]
  Hdr -. useContext .-> Ctx
  ...
  External --> Body
  External --> Ftr
  Note[규칙: 서브 컴포넌트는<br/>Root 없이도 동작해야 함<br/>기본 variant fallback 제공]
```
...
function CardRoot({
  ...
  return (
    <CardContext.Provider value={{ variant }}>
      <div className={cn('rounded-lg p-4', variant === 'outlined' && 'border')}>{children}</div>
...
<Disclosure>
  {({ isOpen, toggle }) => (
    <div>
      <button onClick={toggle}>{isOpen ? '접기 ▲' : '펼치기 ▼'}</button>
      {isOpen && (
        <div className="mt-2 p-4 bg-gray-50 rounded">숨겨진 콘텐츠가 여기에 표시됩니다.</div>
      )}
    </div>
  )}
</Disclosure>
...
> **일상 비유**: 옷장이 어수선할 때 정리 기준이 필요하듯, 코드 리뷰에서 "이건 어디로 옮겨야 하지?" 질문이 잦다면 아래 결정 트리로 답을 좁힙니다.

#### 아키텍처 안티패턴 분류 결정 트리

이 그림은 PR에서 의심 가는 코드 배치를 발견했을 때 어떤 안티패턴인지 판별하는 흐름을 보여줍니다.

```mermaid
flowchart TD
  Start[의심 코드 발견] --> Q1{인터페이스/어댑터/팩토리가<br/>한 구현체에만 쓰임}
  Q1 -- 예 --> A1[8.1 과도한 추상화<br/>직접 import로 환원]
  Q1 -- 아니오 --> Q2{shared/lib에<br/>비즈니스 단어 존재}
  Q2 -- 예 --> A2[8.2 잘못된 계층 배치<br/>domain feature로 이동]
  Q2 -- 아니오 --> Q3{feature 폴더에<br/>독립 변경 가능한 단위 2개 이상}
  Q3 -- 예 --> A3[8.3 거대 feature<br/>분리 필요]
  Q3 -- 아니오 --> Q4{index.ts 미통과<br/>deep import}
  Q4 -- 예 --> A4[8.4 Public API 우회<br/>boundary lint로 차단]
  Q4 -- 아니오 --> Q5{컴포넌트 최상단에<br/>use client 남용}
  Q5 -- 예 --> A5[8.5 use client 남용<br/>경계를 잎 컴포넌트로 이동]
  Q5 -- 아니오 --> Q6{FSD 6계층이<br/>소규모 프로젝트에 적용}
  Q6 -- 예 --> A6[8.6 FSD 오버 엔지니어링<br/>co-location으로 축소]
  Q6 -- 아니오 --> Ok[합격 -- 다른 가이드 절도 확인]
```
...
| 구조 변경 제안서   | 현재 구조, 대안, migration cost  | 제안서 초안             | [15. 의사결정](./15_RFC_의사결정_프로세스.md) 기준 검토 |
...
- `P1(7일 내)` — feature boundary와 shared 승격 조건 중 하나를 작은 변경 1건에 적용하고 증거(import boundary 결과)를 남긴다.
- `P2(30일 내)` — 아키텍처 경계 기준을 팀 템플릿, 체크리스트, CI 중 한 곳에 고정한다.
- `P3(90일 내)` — deep import 수, shared 변경 동반 PR 수, dead code 제거량 추이를 보고 기준을 유지할지 조정할지 결정한다.
- `완료 기준` — 아키텍처 오너가 증거와 철회 조건을 확인했다는 기록을 남긴다.

## 추천 항목 실행 체크리스트

- [ ] `1단계(7일)` : feature boundary와 shared 승격 조건 적용 대상을 1개로 좁힌다.
- [ ] `2단계(30일)` : 증거(import boundary 결과, 폴더 이동 diff, owner 승인)를 PR, ADR, 회고 중 한 곳에 연결한다.
- [ ] `3단계(60일)` : deep import 수, shared 변경 동반 PR 수, dead code 제거량가 기준 안에 들어왔는지 확인한다.
- [ ] `문제 대응` : 미달성 사유와 다음 조치, 중단 여부를 같은 기록에 남긴다.

## 추천 항목 실행 운영 규칙

- `실행 게이트` : 함께 바뀌는 파일이 같은 경계 안에 있는지 먼저 확인한다.
- `승인 체계` : 아키텍처 오너가 영향 범위와 rollback 담당자를 적용 전에 확인한다.
- `재개 조건` : boundary lint와 관련 테스트가 통과하면 다음 feature로 확장한다.
- `정지 조건` : shared가 도메인 분기를 품거나 owner가 불분명하면 승격을 보류한다.
- `리스크 점수` : 영향 feature 수, public API 수, 이동 파일 수로 산정한다.
- `리더 승인자` : 아키텍처 리드가 최종 승인 책임을 맡는다.
- `승인 역할` : 아키텍처 경계 작성자, 검토자, 운영 확인자를 분리해 기록한다.
- `재평가 주기` : 분기마다 import graph와 shared 후보를 다시 본다.
