# 06. 웹 보안 심화 가이드 (2025-2026 Edition)
...
| **핵심 테마** | XSS/CSRF Defense, CSP, JWT Security, CORS, Supply Chain, Secret Management | **Update** | 2025.04 |
...
현대 브라우저의 기본값이지만, 명시적으로 설정하여 외부 사이트에서의 요청을 제한합니다.

| 값 | 동작 | 적합한 경우 |
| :--- | :--- | :--- |
| `Strict` | 외부 사이트에서 오는 모든 요청에 쿠키 미전송 | 은행, 결제 등 높은 보안 필요 |
| `Lax` | GET 네비게이션은 허용, POST 등은 차단 | 일반적인 웹 서비스 (권장) |
| `None` | 모든 요청에 쿠키 전송 (`Secure` 필수) | 서드파티 임베드 필요 시 |
...
## 6. 의존성 보안: Supply Chain Attack 방어

npm 패키지를 통한 공급망 공격(Supply Chain Attack)이 증가하고 있습니다. Lockfile 무결성, 패키지 출처 검증, 설치 스크립트 제어가 핵심입니다.
...
| `X-Frame-Options` | 클릭재킹 방지 (iframe 삽입 차단) | `DENY` 또는 `SAMEORIGIN` |
...
| `Cross-Origin-Opener-Policy` | 팝업과 오프너 간 격리 | `same-origin` |
| `Cross-Origin-Embedder-Policy` | 리소스 임베드 정책 | `require-corp` |
