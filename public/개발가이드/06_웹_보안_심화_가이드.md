...
| 분류 | 아키텍처 | 상태 | Stable |
| :--- | :--- | :--- | :--- |
| **연관 가이드** | [10. 인프라](./10_인프라_및_AWS_CDK_가이드.md), [05. API 통신](./05_API_통신_및_모킹_가이드.md), [11. CI/CD](./11_CICD_파이프라인_표준.md) | **AI 도구** | Claude Code, OWASP ZAP |
...
## 0. OWASP Top 10:2025 — 무엇이 달라졌나

OWASP Top 10은 2025년 11월 Global AppSec(워싱턴 D.C.)에서 공개되어 **2026년 1월 정식 발표**되었습니다. 17.5만 건 이상의 CVE와 실무자 설문을 기반으로 한 사상 최대 규모의 데이터셋으로 갱신되었습니다.
...
| `font-src` | 웹 폰트 소스 제한 | `'self' https://fonts.gstatic.com` |
...
> - **서버 라이브러리**: Node는 `@simplewebauthn/server`, Python은 `webauthn`, Go는 `go-webauthn`이 사실상 표준입니다.
...
| `Lax` | GET 네비게이션은 허용, POST 등은 차단 | 일반적인 웹 서비스 (권장, 사실상 기본값) |
...
# NEXT_PUBLIC_AWS_SECRET_KEY=...    → 브라우저에서 볼 수 있음!
...
const securityHeaders = [
  ...
  },
  {
    ...
    value: [
      ...
      "usb=()",               // USB 접근 금지
      "interest-cohort=()",   // FLoC(Google 광고 추적) 거부
    ].join(", "),
  },
  {
...
> **Prompt 1 — 인증 로직 보안 리뷰**:
> "/analyze src/auth - 이 인증 로직에서 보안 취약점이 있는지 검토해줘. 특히 토큰 저장 방식, 리프레시 로직의 경쟁 상태(Race Condition), 그리고 탈취 방지를 위한 추가적인 가드레일이 필요한 지점을 지적해줘."

> **Prompt 2 — CSP 정책 생성**:
> "현재 프로젝트에서 사용하는 외부 스크립트, 스타일, 이미지 출처를 분석하고 최적의 CSP 헤더를 생성해줘."

> **Prompt 3 — 의존성 보안 점검**:
> "package.json의 의존성 중 알려진 취약점이 있거나, 더 안전한 대안이 있는 패키지를 찾아줘."
