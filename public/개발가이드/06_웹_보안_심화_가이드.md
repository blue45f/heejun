...
| **핵심 테마** | OWASP Top 10:2025, CSP3 + Trusted Types, Passkeys/WebAuthn, RFC 9700 OAuth 2.0 Security BCP, Supply Chain | **Update** | 2026.05 |
...
보안 문서에서 버전명과 표준 상태가 섞이면 잘못된 구현 결정을 만들 수 있습니다. 2026-05 기준 OAuth 2.1은 RFC가 아니라 개발 중인 통합 문서이므로, production 보안 기준은 **RFC 9700 OAuth 2.0 Security BCP**와 각 인증 제공자의 OIDC 문서로 판단합니다.
...
- **보안 표면 최소화**: 쿠키 전략, CSP/Trusted Types, CSRF·CSP 위반 알림을 릴리즈 전 게이트로 묶고 예외를 문서 근거로만 허용합니다.
- **민감정보 격리**: AI 활용, 로깅, 디버그 도구에서 마스킹 정책을 표준 규격으로 두고, 토큰·PII 노출 스캔을 자동화합니다.
- **공급망 보안 연동**: SLSA/SSDF 증적, dependency lockfile 일치, SBOM 변경 이력을 배포 승인 단계와 연결합니다.
- **브라우저 보안 전환 감시**: passkeys/WebAuthn, COOP/COEP 정책 변경 등 주요 보안 변경은 문서 트리거 항목으로 등록해 24h 내 리스크 평가합니다.
...
- 비밀관리 제로 노출: CI/로컬 로그에서 비밀 마스킹을 강제하고 유출 의심 시 회수 포인트까지 추적합니다.
- CSP/COEP/COOP 고도화: 보안 헤더는 보수적 기본값으로 운영하고 호환성 이슈는 단계적으로 완화합니다.
- 공급망 인증 분리: dependency audit와 SBOM/attestation 결과를 릴리즈 승인 조건으로 묶고 임계치 초과 시 블록합니다.
- passkeys·세션 전환 전략: WebAuthn 도입은 세션 탈취·복구 플로우와 함께 단계적으로 확장합니다.
- 보안 사고 증적 연결: 중요 이벤트는 운영 대응 라인과 Evidence Contract에 함께 기록해 재발 여부를 검증합니다.

- 인증/권한 로직의 최신 위협 모델을 기준선으로 삼고, 토큰 회전·재발급 실패가 서비스 흐름에 미치는 영향을 모의 장애로 검증합니다.
- 공급망 경로는 lockfile, SBOM, attestation까지 한 번에 관리해 의존성 변경이 보안 정책을 우회하지 않도록 통합합니다.
- 취약점 대응은 버전 고정/폐기 임계치와 연결해 단일 결함이 다중 시스템으로 확산되지 않도록 규칙화합니다.
- 보안 강건성은 기능 릴리즈 이전 보안 테스트 패키지와 연동하고, 임계값 위반 시 해당 기능을 기본 off로 되돌리는 절차를 선제 준비합니다.
- 공급망 감시의 회귀 원인을 추적하는데 쓰는 태그 규약을 문서화해 사고 분석 시간이 커지는 것을 방지합니다.
- 보안 규칙은 “실제 침해사고 대응 속도”와 연동되어 주기적으로 실전 워게임 형태로 갱신합니다.
- 신기능 도입은 취약점 스캐너와 규정 준수 점검에서 연속 통과 이력을 확보한 경우에만 기본값 전환합니다.
- 보안 게이트는 기능별 영향도에 따라 이중 검증을 적용하고 고위험 구간은 기본 비활성으로 운영합니다.
- 모의 침해 공격 시나리오를 문서 업데이트 주기와 동기화해 대응시간을 정량 관리합니다.
...
- **민감권한 가드**: 비인가 경로와 민감 데이터 접근은 증거 없는 자동 채택을 금지합니다.
- **공급망 즉시 조치**: 취약점 경보는 severity별 즉시 대응 표준으로 자동 라우팅합니다.
...
- [ ] **보안 정책 검증**: 보안 게이트 변경이 공급망 증적과 연동되는지 확인한다.
- [ ] **탐지 규칙 훈련**: 공격 시뮬레이션 결과를 학습 큐로 즉시 이관한다.